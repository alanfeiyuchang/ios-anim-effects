...
extension Effect {
    static let chartsRadarMorph = Effect(
        ...
        summary: L("A six-axis radar polygon that springs between datasets while its colour cross-fades.", "六轴雷达多边形在数据集之间弹性形变，颜色同步渐变。"),
        prompt: L(
            "A 200 pt six-axis radar chart: four concentric hexagonal guides and six spokes in 8–12% primary, axis labels in caption type just outside the rim, and a data polygon filled with its dataset colour at 25% plus a 2 pt stroke and 7 pt vertex dots with white rims. A segmented control of three pills (v1.0, v2.0, v3.0) sits below, its selection capsule sliding between pills via matched geometry. Choosing a dataset morphs every vertex simultaneously to its new radius on one spring (response ≈ 0.6 s, damping ≈ 0.65), so the shape swells, pinches and overshoots slightly like an elastic membrane, while fill and stroke cross-fade to the new colour over the same timing. A selection haptic confirms each switch. Organic, comparative and instantly readable.",
            "一张 200pt 的六轴雷达图：四圈同心六边形参考线与六条辐射轴为 8–12% 主色，轴标签以说明字号置于外圈之外；数据多边形以所属数据集颜色 25% 填充，配 2pt 描边与带白边的 7pt 顶点圆点。下方是由三个胶囊组成的分段控件（v1.0、v2.0、v3.0），选中胶囊借助几何匹配在选项间滑动。切换数据集时，所有顶点在同一个弹簧（响应约 0.6 秒、阻尼约 0.65）中同时移动到新半径，图形像弹性薄膜一样膨胀、收缩并轻微过冲；填充与描边在同一节奏中渐变到新颜色。每次切换伴随选择触感。有机、便于对比、一目了然。"
        ),
        implementation: L(
