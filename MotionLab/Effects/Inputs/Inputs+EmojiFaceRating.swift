...
extension Effect {
    static let inputsEmojiFaceRating = Effect(
        ...
        prompt: L(
            ...
            "反馈卡片上方是一张 120pt 的脸，下方是五档标签滑块（很差 → 很棒）。拖动时脸连续形变：嘴是一条贝塞尔曲线，控制点从深皱眉移到咧嘴笑；心情越好眼睛眯得越短；低于中档时眉毛内端上扬显出担忧，高于中档则放平；脸色由珊瑚红经琥珀过渡到薄荷绿。松手后以弹性弹簧（响应 0.45 秒、阻尼 0.6）吸附到最近一档，嘴形先过冲再定型；每换一档，脸以关键帧上跳 6pt，标签推入切换，并触发选择触觉。表情丰富，绝不会被误读。"
        ),
        implementation: L(
