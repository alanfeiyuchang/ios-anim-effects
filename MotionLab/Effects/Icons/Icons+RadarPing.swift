...
extension Effect {
    static let iconsRadarPing = Effect(
        ...
        prompt: L(
            "A location glyph sits in a glossy blue disc at the centre of a faint circular grid. Three sonar rings leave the disc one after another, each expanding from 36 pt to the full 250 pt radar over 2.4 s on an ease-out curve while fading from 45% to 0, so a new ring is always on its way. A conic beam with a bright leading edge and a 70° fading tail rotates once every 3 s. Five 'nearby device' dots sit at fixed bearings: when the beam passes a dot it flares to 150% with a glow, then decays over ~0.8 s. The central disc breathes ±4% in sync with each ring. Calm, continuous and alive — the feeling of the device quietly searching.",
            "定位图标放在一个带光泽的蓝色圆盘里，位于淡淡的圆形网格中央。三道声呐波纹依次从圆盘发出，每道都在 2.4 秒内以缓出曲线从 36 pt 扩散到整个 250 pt 的雷达范围，同时透明度从 45% 降到 0，因此始终有新的波纹在路上。一道锥形扫描光束带着明亮的前缘和 70° 的渐隐尾迹，每 3 秒旋转一圈。五个「附近设备」小点固定在不同方位：光束扫过时小点放大到 150% 并发光，再在约 0.8 秒内衰减。中央圆盘随每道波纹做 ±4% 的呼吸。平静、持续、富有生命感——设备正在安静地搜寻。"
        ),
        implementation: L(
