...
extension Effect {
    static let loadingDotBounce = Effect(
        ...
        prompt: L(
            "Three 14 pt dots in indigo, violet and pink sit in a row with 8 pt gaps. Each dot hops on a half-sine arc — rising ~16 pt and falling back within the first half of a 1.1 s cycle, then resting — with a ~155 ms (14% of a cycle) stagger so the motion travels left to right like a wave; at the top of a hop a dot reaches 100% scale and full opacity, at rest it shrinks to 85% and dims to 50%. By default the row lives in a real chat thread: an incoming bubble with a 6 pt tail corner, a hairline border and soft shadow that breathes 1.00 → 1.03 every 1.1 s, beneath a gradient outgoing message and a 'Mia is typing…' caption. Light, friendly and never frantic.",
            "三颗 14 pt 圆点（靛蓝、紫罗兰、粉色）横向排列，间距 8 pt。每颗圆点在 1.1 秒周期的前半段沿半正弦弧线起跳约 16 pt 再落回，后半段静止；相邻圆点错开约 155 毫秒（14% 个周期），形成从左向右传递的波浪；跳到最高点时恢复 100% 大小与完全不透明，静止时缩到 85%、透明度降到 50%。默认放在真实的聊天场景里：一枚带 6 pt 尖角、细描边与柔和投影的接收气泡，每 1.1 秒在 1.00 → 1.03 之间轻轻“呼吸”；上方是一条渐变的已发送消息和“米娅正在输入…”小字。节奏轻盈、友好，绝不急躁。"
        ),
        implementation: L(
...
extension Effect {
    static let loadingActivityPetals = Effect(
        ...
        prompt: L(
            "Twelve rounded capsule petals (3.5 × 11 pt) radiate from the hub, centered 13 pt out, like the iOS activity indicator. A bright 'head' travels clockwise once per second; each petal is fully opaque when the head passes and fades back to an 18% resting opacity over the following ~80% of a lap, producing a trailing comet of light. In stepped mode the head jumps petal by petal, exactly like UIActivityIndicatorView; in smooth mode it glides. By default it sits at 1.5× inside a 148 pt frosted HUD with 28 pt continuous corners and a 'Loading' caption, floating over a photo grid so the material visibly blurs what's behind it. The color follows the label color for light and dark mode.",
            "十二片圆角胶囊花瓣（3.5 × 11 pt）以 13 pt 中心半径呈放射状排列，形如 iOS 系统加载指示器。一个“高亮头”每秒顺时针转一圈；它经过时花瓣完全不透明，随后在约 80% 圈的时间里淡回 18% 的静息透明度，形成拖尾光带。逐格模式下高亮头一瓣一瓣跳动，与 UIActivityIndicatorView 完全一致；平滑模式下连续滑行。默认以 1.5 倍大小放在一块 148 pt、28 pt 连续圆角的磨砂 HUD 中，下方写着“正在加载”，悬浮在一组照片网格之上，让材质对背景的模糊清晰可见。颜色跟随系统文字色，深浅模式都清晰。"
        ),
        implementation: L(
