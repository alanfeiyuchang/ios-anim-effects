...
extension Effect {
    static let backgroundsInkBloom = Effect(
        ...
        prompt: L(
            ...
            "纸白色画面（深色模式为墨黑）。每次点击在指尖处滴下一滴墨：约 160 毫秒内先出现浓重的墨核，随后七团柔软的墨瓣沿随机方向向外推开并长大，遵循 1 − e^(−t/0.9 秒) 的扩散曲线——起初迅速、随后慢到几乎静止；墨瓣同时受重力下沉（与扩散进度成二次方关系），被拉成墨丝。滴落处有一圈淡淡的波纹扩散淡出。所有墨色都经 10pt 模糊，并在纸面上以正片叠底（深色模式下为滤色）混合，靛蓝、洋红、青绿与赭黄的墨滴重叠时如真实颜料般交融。每滴墨在生命周期内（默认 7 秒）按二次方淡出。有机、手作感。"
        ),
        implementation: L(
    ...
    static let backgroundsSloshTank = Effect(
        ...
        prompt: L(
            "The stage is a tank of water filled to 55%: a translucent back surface and a brighter mint-to-blue front surface, each a live curve with a thin white crest, over a deep navy backdrop with small bubbles rising through the water. The surface tilt is a spring (stiffness 55, damping ratio 0.12 by default): tapping or dragging sideways sets a target slope that piles water toward the finger, and letting go (or the end of a tap's brief poke) releases it, so the surface swings past level several times before settling. The tilt velocity also drives a second, S-shaped slosh mode (up to ±24 pt), the back surface follows at 82% of the tilt, and fine ripples keep moving on top. A soft haptic marks each release. Playful, weighty and liquid.",
            "整个舞台是一只注水 55% 的水箱：后层半透明水面与更明亮的薄荷绿至蓝色前层水面，都是带白色细波峰的实时曲线；背景为深海军蓝，小气泡在水中缓缓上升。水面倾斜由弹簧驱动（默认刚度 55、阻尼比 0.12）：点击或横向拖动会设定目标斜率，使水向手指一侧堆高；松手或轻点结束后弹簧释放，水面会越过水平线来回摆动好几次才平息。倾斜速度还会激发第二个 S 形晃动模态（最多 ±24pt），后层水面以 82% 的倾斜跟随，细小涟漪始终在表面流动。每次松手伴随一次柔和触感。俏皮、有分量、充满液体感。"
        ),
        implementation: L(
