...
extension Effect {
    static let feedbackReactionPicker = Effect(
        ...
        prompt: L(
            "A short chat thread. Tapping the incoming message lifts it to 103% with a deeper shadow while the neighbouring bubbles blur to 3 pt and fade to 45%, putting it in focus. A frosted capsule of six emoji reactions blooms from the bubble's top-leading corner — scaling from 40% on a lively spring (response 0.4 s, damping 0.72) — and each emoji pops in from 20% scale and 8 pt lower, staggered 30 ms left to right. Pressing an emoji swells it to 135% and lifts it 6 pt. Choosing one fires a success haptic, collapses the bar, and the emoji itself flies along a shared-element path onto the bubble's top-trailing corner, landing in a small elevated badge; reopening sends it back into the bar. Expressive, precise and warm.",
