...
extension Effect {
    static let gesturesNotchSlide = Effect(
        ...
        prompt: L(
            ...
            "一条 290×64 pt 的胶囊滑轨被 3 pt 间隙分成四段，上方是步骤标签（核对 · 验证 · 授权 · 支付），左侧是带银行卡图标的 56 pt 白色滑块。拖动在每个刻口附近带磁性：距刻口 18 pt 以内，滑块偏移按二次曲线缓动，像被吸住，再拉才追上手指，每越过一个刻口都有一下硬朗的“咔哒”触感。越过的分段以挤压弹跳（弹簧响应 0.3 秒、阻尼 0.5）填成靛蓝，对应标签变粗。拖到终点松手即提交：各段从左到右间隔 60 毫秒依次变绿，图标换成对勾，伴随成功触感；中途松手则滑块弹回，分段从右到左熄灭。干脆可靠。"
        ),
        implementation: L(
