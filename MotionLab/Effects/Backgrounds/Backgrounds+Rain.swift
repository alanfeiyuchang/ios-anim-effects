...
extension Effect {
    static let backgroundsRain = Effect(
        ...
        prompt: L(
            "A moody night-city backdrop: a deep blue-slate gradient with a handful of heavily blurred amber, coral and cyan window lights glowing near the bottom. Over it, hundreds of fine rain streaks fall at a wind-set slant; each streak's speed, length, width and opacity scale with a random depth value, so near drops are brighter, longer and faster while distant ones read as a faint haze. At the base, small flattened ripple ellipses expand and fade on staggered ~0.8 s cycles where drops land. Tapping triggers a lightning strike: an instant cold-white flash, a brief dip, a second flicker at ~240 ms, then an exponential fade over ~0.5 s with a heavy haptic. Cinematic, melancholic and cosy.",
            "情绪化的夜城背景：深蓝灰渐变，底部点缀几团高度模糊的琥珀色、珊瑚色与青色窗灯。其上数百条细密雨丝按风向倾斜坠落；每条雨丝的速度、长度、粗细与透明度都随随机景深值变化，近处雨滴更亮、更长、更快，远处则化为一层淡淡雨雾。画面底部，雨滴落点处的扁平涟漪椭圆以约 0.8 秒的错峰周期扩散并淡出。点击触发闪电：先是瞬间的冷白闪光，短暂变暗后在约 240 毫秒处再闪一次，随后在约 0.5 秒内指数衰减，并伴随强烈触觉反馈。电影感十足，忧郁又温馨。"
        ),
        implementation: L(
