...
extension Effect {
    static let textVariableWeight = Effect(
        ...
        prompt: L(
            "A large display word in the system variable font breathes through its weight axis: a smooth sine wave travels left to right across the letters, each glyph easing continuously between Thin (100) and Black (900) with a phase lag of ~0.55 rad per letter, one full cycle every ~1.25 s. Because weights interpolate rather than step, the letters swell and slim like a single elastic body and the word's width ripples as advances change. Heavier glyphs warm from indigo to coral. Dragging across the word takes over: the weight peaks under the finger and falls off with a Gaussian over about two letters, blending in over 250 ms and handing back to the wave over 400 ms on release. A live “wght” readout of the word's average weight ticks beneath in monospaced digits. Typographic, tactile and quietly hypnotic.",
            "一个大号展示单词使用系统可变字体，沿字重轴“呼吸”：一道平滑的正弦波从左向右穿过各个字母，每个字形在 Thin（100）与 Black（900）之间连续过渡，相邻字母相位滞后约 0.55 弧度，约 1.25 秒完成一个周期。字重是连续插值而非跳档，字母像一个有弹性的整体般膨胀、收细，单词宽度也随字宽变化而起伏。越粗的字形颜色越从靛蓝暖化为珊瑚色。在单词上拖动时由手指接管：字重在指尖处达到峰值，并按约两个字母宽的高斯曲线向两侧衰减，250 毫秒内平滑接入，松手后在 400 毫秒内交还给波浪。下方以等宽数字实时显示整词平均字重的“wght”数值。兼具字体设计感与触感，安静而令人着迷。"
        ),
        implementation: L(
