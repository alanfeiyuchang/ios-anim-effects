...
extension Effect {
    static let loadingPulseRings = Effect(
        ...
        prompt: L(
            "A 72 pt gradient core with a white antenna glyph sits at the center. Three rings emit from behind it, evenly staggered across a 2.2 s period: each grows from the core's edge to 2.6× on a quadratic ease-out while its 18% tinted fill and 60% 1.5 pt hairline fade to nothing, and the core breathes ±3%. Over a 6 s scene, three nearby-device avatars (40 pt, white rim) pop onto the radar 1.1 s apart on a back-out curve with about 10% overshoot, then fade together as the search restarts. Calm and hopeful, like AirDrop discovery.",
            "中心是一枚 72 pt 渐变核心，内含白色天线图标。三圈圆环在 2.2 秒周期里均匀错峰地从它背后荡出：每圈以二次缓出从核心边缘扩到 2.6 倍，18% 的着色填充与 60% 的 1.5 pt 细描边一同淡尽；核心随之轻轻呼吸（±3%）。6 秒一轮的场景中，三枚附近设备头像（40 pt、白边）每隔 1.1 秒以约 10% 过冲的回弹曲线“啵”地冒上雷达，随后一齐淡出，搜索重新开始。平静而期待，像隔空投送在寻找对方。"
        ),
        implementation: L(
