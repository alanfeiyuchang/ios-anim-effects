...
extension Effect {
    static let buttonsDepthPress = Effect(
        ...
        prompt: L(
            "A chunky, game-like button built from two stacked layers: a saturated gradient face and a darker base of the same hue peeking out 8 pt below it, giving a solid extruded look with a soft ground shadow. On touch-down the face travels straight down about 85% of the depth in ~120 ms on a stiff, near-critically-damped spring, the visible base collapses and the ground shadow shrinks and tightens, so the key reads as pushed in, and a medium impact haptic lands as it bottoms out. On release the face pops back up on a bouncier spring (response 0.35 s, damping 0.55) with a tiny overshoot. It feels mechanical, chunky and satisfying, like a keyboard key or arcade button.",
            "由两层叠成的厚实游戏风按钮：上层是饱和渐变的面板，下层是同色系更深的底座，从面板下方露出 8pt，形成立体挤出感，并带柔和的落地投影。手指按下时，面板在约 120 毫秒内以接近临界阻尼的硬弹簧垂直下沉约 85% 的厚度，露出的底座随之消失，落地投影缩小收紧，读起来就是“按进去了”，触底瞬间伴随一次中等强度的触觉反馈。松手时面板以更有弹性的弹簧（响应 0.35 秒、阻尼 0.55）弹回并轻微过冲。手感机械、厚实、爽快，如同键盘键帽或街机按钮。"
        ),
        implementation: L(
