...
extension Effect {
    static let textOdometer = Effect(
        ...
        prompt: L(
            ...
            "机械里程表以六个深色圆柱滚轮显示 4231.8 公里——五个整公里滚轮、一个琥珀色小数点和一个琥珀色的十分位滚轮。每个滚轮是一条无限循环的 0–9 数字带，只露出一个窗口，上下内阴影模拟圆柱曲面。读数增加时，各滚轮只会向前滚动（9 之后接着滚到 0，绝不倒转经过 8…1），以轻度欠阻尼弹簧（响应约 0.8 秒、阻尼 0.78）停到新数字，并从右向左依次错开约 60 毫秒，进位像真实齿轮逐级咬合般传递；十分位滚轮转得最快。每次滚动伴随轻微冲击触感——触感真实、复古而令人满足。"
        ),
        implementation: L(
            "Each drum is an Animatable view whose animatableData is an unbounded step count (value ÷ 10^column); it draws only the two digits around the window, offset by the fractional part, and animates with .animation(.spring(...).delay(column × stagger), value: steps).",
            "每个滚轮是一个 Animatable 视图，其 animatableData 为无上限的累计步数（读数 ÷ 10^列）；它只绘制窗口附近的两个数字并按小数部分偏移，使用 .animation(.spring(...).delay(列序 × 错开), value: 步数) 动画。"
        ),
...
private struct OdometerDemo: View {
    ...
    private func roll(at column: Int) -> Int {
