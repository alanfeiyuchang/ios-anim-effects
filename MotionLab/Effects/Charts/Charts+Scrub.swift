...
extension Effect {
    static let chartsScrub = Effect(
        ...
        prompt: L(
            "A 24-hour line chart (Swift Charts, Catmull-Rom line 2.5 pt in blue over a blue-to-clear gradient area, hidden y-axis, hour labels every 6 h) inside a card whose header shows the current value in large rounded digits. Touching and dragging horizontally selects the nearest hour: a dashed 1 pt vertical rule appears, a 9 pt point with a white ring sits on the curve, and a compact material tooltip (time + value) floats above the chart, clamped so it never leaves the plot. The header number rolls to the selected value with a numeric content transition, every hour boundary crossed produces a selection haptic tick, and all marks glide with a snappy 250 ms animation rather than jumping. Lifting the finger clears the selection. Precise and tactile, like the Stocks and Health apps.",
            "一张 24 小时折线图（Swift Charts，2.5pt 蓝色 Catmull-Rom 曲线，下方为蓝色到透明的渐变面积，隐藏 y 轴，每 6 小时一个时间标签），置于卡片中，卡片标题以大号圆体数字显示当前数值。手指按下并水平拖动时选中最近的整点：出现一条 1pt 虚线竖向参考线，一个带白色描边的 9pt 数据点贴在曲线上，上方悬浮一个紧凑的材质提示框（时间 + 数值），并自动约束在绘图区内。标题数字以数字内容转场滚动到所选数值，每跨过一个整点触发一次选择触感，所有标记以 250ms 的利落动画滑行而非跳变。抬起手指即取消选择。精准、可触，与“股市”“健康”App 同款体验。"
        ),
        implementation: L(
