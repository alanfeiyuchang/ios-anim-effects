...
extension Effect {
    static let gesturesPiPSnap = Effect(
        ...
        prompt: L(
            ...
            "一个104 × 66pt的悬浮视频小窗（14pt连续圆角、紫→蓝→天蓝渐变、播放图标与进度条、深而柔和的投影）以12pt边距停靠在手机屏幕的某个角落。拖拽时小窗跟手并放大到105%，四个角落槽位以淡淡的虚线框浮现，预测目标槽位同步填充浅色。松手时，根据手势预测速度推算落点（当前位置+动量系数×（预测位置−当前位置）），取最近的角落，小窗以弹簧（响应约0.5秒、阻尼约0.78）飞向该处，并伴随中等触感。轻轻一甩即可横跨屏幕，与iOS画中画的手感一致。"
        ),
        implementation: L(
