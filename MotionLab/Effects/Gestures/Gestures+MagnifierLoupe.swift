...
extension Effect {
    static let gesturesMagnifierLoupe = Effect(
        ...
        prompt: L(
            "A 12×9 grid of tiny colour swatches (hue across, brightness down, each labelled with a 3 pt hex code too small to read) fills a 300×225 pt panel. Touching it pops a 100 pt circular loupe 78 pt above the finger, springing from 40% to 100% from its bottom edge (response 0.3 s, damping 0.7); near the top edge it flips below the finger, and near the sides it slides inward so the lens never leaves the panel. Inside, the content is re-rendered rather than upscaled at 3× around the touch point, so the hex labels turn crisp, with a thin crosshair on the sampled swatch, a 3 pt white rim, a soft shadow and a capsule beneath showing the colour and its hex value. The loupe tracks the finger with zero latency and shrinks away on lift. Precise, tool-like and a little magical.",
            "一块 300×225 pt 的面板铺满 12×9 的迷你色块（横向变色相、纵向变明度，每块都标着 3 pt、肉眼难辨的十六进制色值）。按下时，一个 100 pt 的圆形放大镜在手指上方 78 pt 处以底边为锚点从 40% 弹到 100%（响应 0.3 秒、阻尼 0.7）；靠近顶部时翻到手指下方，靠近左右边缘时向内平移，镜片始终不出面板。镜内以触点为中心按 3 倍重新绘制而非位图放大，色值文字清晰可读；细十字准星标出取样点，3 pt 白边与柔和投影让镜片浮起，下方胶囊显示色块与色值。放大镜零延迟跟手，抬起手指即缩小消失。精准、专业，又带点魔法感。"
        ),
        implementation: L(
