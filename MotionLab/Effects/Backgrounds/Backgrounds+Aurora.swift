...
extension Effect {
    static let backgroundsAurora = Effect(
        ...
        prompt: L(
            ...
            "夜空场景：由深海军蓝过渡到青绿的纵向渐变背景，点缀约 70 颗独立闪烁（周期约 3–6 秒）的细小星点，底部是一道深色山脊剪影。天空中四条宽幅、强模糊的极光飘带（薄荷绿、青蓝、浅绿、紫粉）横贯摇曳：每条飘带是由两组正弦波叠加而成的描边曲线，相位以略有差异的速率推进（周期约 8–12 秒），因此光幕不断起伏、交错，粗细也随之呼吸。飘带采用叠加混合，两端横向淡出，上方再叠一层更淡的回声，形成垂直的光幕纵深。整体静谧、电影感十足，令人屏息。"
        ),
        implementation: L(
