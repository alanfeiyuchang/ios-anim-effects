...
extension Effect {
    static let buttonsLiquidGlass = Effect(
        ...
        prompt: L(
            ...
            "鲜艳的风景照片卡片上浮着一枚 52pt 橙色调液态玻璃胶囊“收藏到行程”，右上角还有圆形玻璃爱心按钮。触摸任一玻璃按钮都有系统交互式反馈：轻微膨胀，高光随手指移动。点击后胶囊作为一整块玻璃形变为 52pt 的绿色对勾圆，同时从右缘长出一枚“撤销”玻璃胶囊，两者相距约 16pt 前以液态桥相连，随后断开；全程由同一弹簧（响应 0.45 秒、阻尼 0.8）驱动，并触发成功触感。点“撤销”或对勾，两颗液滴又融回胶囊。iOS 26 以下改用磨砂胶囊加渐变描边完成同样编排。"
        ),
        implementation: L(
