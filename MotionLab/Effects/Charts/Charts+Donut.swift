...
extension Effect {
    static let chartsDonut = Effect(
        ...
        prompt: L(
            ...
            "一个 190pt 的环形图，由五段圆角端点弧线组成（描边 26pt，段间留细小角度间隙，配色为靛蓝、粉、琥珀、薄荷绿、天蓝）。出现时整环从 −150° 转到 −90°，各段同时以弹簧（响应 0.7 秒、阻尼 0.8）扫到完整长度，逐段错开 100ms，如折扇般顺时针展开。点击某段即“弹出”：沿角平分线外移 12pt，描边加粗到 34pt 并带同色柔影，其余分段降至 35% 透明度；中心标签从总量切换为该类别名称与占比，数字滚动过渡。下方三列图例同步高亮，也可直接点选。再点该段或中空处，一切以利落弹簧复原。每次切换伴随选择触感，清晰而愉悦。"
        ),
        implementation: L(
