...
extension Effect {
    static let textSevenSegment = Effect(
        ...
        prompt: L(
            "A kitchen-timer display counts down from 10:00 in slanted seven-segment digits on a near-black panel. Every digit is built from seven rounded bars: unlit bars stay faintly visible at 7% so the grid reads as hardware. When a digit changes, bars that switch on ignite fast — 80 ms ease-out, scanning in segment order a→g with 25 ms between bars, each gaining a 6 pt coloured glow — while bars that switch off fade slowly over 350 ms like cooling phosphor, so the old numeral ghosts briefly under the new one. The colon blinks at 1 Hz. Tapping resets the timer with a light haptic. Retro, precise and warm.",
            "一块近乎纯黑的面板上，厨房计时器以倾斜的七段数码管从 10:00 开始倒数。每个数字由七根圆角笔画组成：未点亮的笔画保留 7% 的微光，让整块屏幕看起来像真实硬件。数字变化时，需要点亮的笔画快速亮起——80 毫秒缓出，按 a→g 的笔画顺序扫描，笔画间隔 25 毫秒，并带上 6 pt 的彩色辉光；需要熄灭的笔画则像冷却的荧光粉一样在 350 毫秒内缓缓暗下，旧数字会短暂残留在新数字之下。冒号每秒闪烁一次。点击可重置计时并伴随轻触感。复古、精准又带着温度。"
        ),
        implementation: L(
