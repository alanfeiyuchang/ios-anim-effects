...
extension Effect {
    static let gesturesNewtonsCradle = Effect(
        ...
        prompt: L(
            "Five 36 pt chrome balls (radial highlight from white through silver to graphite) hang side by side from a slim bar on 140 pt strings, each casting a soft floor shadow that shrinks and fades as the ball rises. Every ball is a real pendulum (g ≈ 2000 pt/s², period ≈ 1.7 s, light air drag) integrated at 240 Hz. Dragging any ball swings it on its string arc (limited to ±52°); releasing hands over the finger's tangential speed. Contacts are resolved as near-elastic collisions (restitution ≈ 0.97) in alternating passes, so momentum travels through the resting balls within a few milliseconds and the far ball flies out while the middle ones barely move — two pulled balls send two out. Each hard contact gives a rigid haptic tick. Slowly the energy bleeds away until the row hangs still. Precise, hypnotic, physically faithful.",
