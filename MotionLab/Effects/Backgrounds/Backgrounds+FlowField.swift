...
extension Effect {
    static let backgroundsFlowField = Effect(
        ...
        prompt: L(
            ...
            "近黑的墨色背景上，约 260 个粒子沿看不见的向量场漂流：方向由三组缓慢演变的正弦项叠加而成，洋流弯成宽阔多变的漩涡。每个粒子的轨迹按模拟时间每 1/30 秒采样一次（14 个采样点，约 0.45 秒路径），长度只由速度决定、与帧率无关；轨迹分三段，从暗淡纤细的尾部渐变到明亮稍粗的头部，取四种色相之一（靛蓝、紫、天蓝、薄荷），叠加混合，密集处自然更亮。粒子寿命 2.5–6 秒，生灭都有淡入淡出。横向拖动时手指化身漩涡中心：约 120pt 内的粒子获得切向旋转与轻微外推，绘出发光涡流，松手后融回洋流，安静而高级。"
        ),
        implementation: L(
