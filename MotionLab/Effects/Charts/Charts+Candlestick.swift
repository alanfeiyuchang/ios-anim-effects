...
extension Effect {
    static let chartsCandlestickLive = Effect(
        ...
        prompt: L(
            ...
            "一张交易卡片：顶部是交易对与最新价，随实时蜡烛涨跌染成绿或红；下方是 18 根历史 K 线加 1 根实时 K 线。实时收盘价每 140ms 获得一个随机游走目标，并以约 70ms 的指数跟随趋近，实体与影线自然伸缩而非跳变；虚线价格线与彩色价格标签在右侧价格轴同步跟随，该栏还标注三条水平参考线。实时蜡烛始终占据最右一格并带柔和辉光。周期结束时蜡烛被固定，整组序列连续平滑地左移一格；纵轴在约 200ms 内缓动，以 12% 余量适配可见高低点。点击可注入一次剧烈波动。"
        ),
        implementation: L(
