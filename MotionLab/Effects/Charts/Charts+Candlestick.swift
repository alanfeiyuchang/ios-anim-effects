...
extension Effect {
    static let chartsCandlestickLive = Effect(
        ...
        prompt: L(
            ...
            "一张交易卡片：顶部是交易对名称与最新价，按实时蜡烛涨跌染成绿色或红色；下方是 18 根历史 K 线加 1 根实时 K 线。实时收盘价每 140ms 获得一个新的随机游走目标，并以约 70ms 的指数跟随趋近，实体与影线因此自然伸缩而非跳变；虚线价格线与彩色价格标签在右侧价格轴一栏同步跟随，该栏同时标注三条水平参考线。实时蜡烛始终在最右侧占满一格，并带柔和辉光。周期结束时蜡烛被固定，整组序列连续平滑地左移一格，毫无阶跃；纵轴刻度在约 200ms 内缓动，以 12% 余量适配可见区间的高低点。点击可注入一次剧烈波动，精准而鲜活。"
        ),
        implementation: L(
