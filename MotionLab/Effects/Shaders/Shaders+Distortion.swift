...
extension Effect {
    ...
    static let shaderWave = Effect(
        ...
        prompt: L(
            ...
            "卡片像微风中的旗帜一样持续起伏。每个像素在竖直方向受沿 x 轴传播的正弦波推移，在水平方向受沿 y 轴、更慢且振幅减半的余弦波推移，表面呈斜向涟漪，而非整体上下跳动。波形斜率同时为布面打光：迎光面最多提亮约 30%，背光面相应变暗，褶皱富有布料的体积感。运动无缝循环、无缓动：振幅约 6pt，波长参数 30pt（波峰间距约 190pt），周期约 2 秒。横向拖动就是风：振幅随拖动距离增大，最多到 2.5 倍；松手后以松弛的弹簧（响应 0.8 秒、阻尼 0.35）回弹，带着旗面甩动般的过冲。平静而催眠。"
        ),
        implementation: L(
