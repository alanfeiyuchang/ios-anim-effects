...
extension Effect {
    static let inputsRockerSwitch = Effect(
        ...
        prompt: L(
            "A hardware-style rocker switch (132 × 72 pt paddle, marked O and I) seated in a recessed bezel on a device card. Tapping presses the whole housing to 97%, then the paddle tips 12° around its vertical axis with strong perspective on a very stiff, nearly critically damped spring (response 0.18 s, damping 0.9) — no wobble, just a decisive clack with a rigid haptic. Lighting follows the tilt: the raised half catches a highlight gradient while the pressed half falls into shadow, and the bezel's inner shadow shifts sides. A pilot LED then flickers on (0 → 100% → 40% → 100% over 260 ms) and blooms a soft amber glow; switching off kills the LED instantly. Mechanical, weighty, satisfying.",
            "一块设备面板上嵌着一枚硬件风格的翘板开关（132 × 72pt 按板，标有 O 与 I），四周是下凹的边框。点击时整个外壳先压到 97%，随后按板以强透视绕竖轴翻转 12°，采用极硬、接近临界阻尼的弹簧（响应 0.18 秒、阻尼 0.9）——没有晃动，只有一声干脆的“咔嗒”和一次硬朗触觉。光影随倾角变化：翘起的一半受光出现高光渐变，按下的一半落入阴影，边框内阴影也随之换边。随后指示灯以闪烁方式点亮（260 毫秒内 0 → 100% → 40% → 100%），晕开柔和的琥珀色光；关闭时指示灯瞬间熄灭。机械、厚重、令人满足。"
        ),
        implementation: L(
