...
extension Effect {
    static let chartsRadarMorph = Effect(
        ...
        prompt: L(
            ...
            "一张 196pt 的六轴雷达图：四圈同心六边形参考线与六条辐射轴为 8–12% 主色，轴标签置于外圈之外；数据多边形以所属数据集颜色 25% 填充，配 2pt 描边与带同色光晕的 7pt 顶点圆点。下方是三个胶囊组成的分段控件（v1.0、v2.0、v3.0），选中胶囊借助几何匹配滑动。切换数据集时，所有顶点在同一个弹簧（响应约 0.6 秒、阻尼约 0.65）中同时移到新半径，图形像弹性薄膜一样膨胀、收缩并轻微过冲；填充与描边在同一节奏中渐变到新颜色。每次切换伴随选择触感。有机、便于对比。"
        ),
        implementation: L(
