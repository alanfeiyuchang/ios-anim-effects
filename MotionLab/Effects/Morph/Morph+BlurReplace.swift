...
extension Effect {
    static let morphBlurReplace = Effect(
        ...
        prompt: L(
            ...
            "一枚类似灵动岛的黑色胶囊，以及其下方的大号状态卡片，依次循环展示实时状态：AirPods 已连接、计时进行中、支付完成、专注模式开启。每次切换时，旧的图标与文字在模糊中缩小、轻微位移后消散，新内容从相反方向由模糊变清晰，两者短暂重叠，形成连贯的溶解过渡而非生硬切换。同时胶囊以灵敏的弹簧（响应约 0.45 秒、阻尼约 0.8）伸缩以贴合新内容，卡片中的 SF Symbol 使用原生替换特效切换，强调色随之过渡。整体像同一块「活」的表面在重组，而非幻灯片翻页。"
        ),
        implementation: L(
