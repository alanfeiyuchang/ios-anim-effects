...
extension Effect {
    static let chartsScrub = Effect(
        ...
        prompt: L(
            ...
            "一张 24 小时折线图（Swift Charts，2.5pt 蓝色 Catmull-Rom 曲线，下方为蓝到透明的渐变面积，隐藏 y 轴，每 6 小时一个时间标签），卡片标题以大号圆体数字显示当前值。手指水平拖动时选中最近的整点：出现 1pt 虚线竖向参考线，一个带 2.5pt 白描边的 11pt 数据点贴在曲线上，上方悬浮紧凑的材质提示框（时间 + 数值），并约束在绘图区内。标题数字以数字转场滚到所选值，每跨一个整点触发选择触感，所有标记以 250ms 的利落动画滑行而非跳变。抬手即取消选择，精准可触。"
        ),
        implementation: L(
