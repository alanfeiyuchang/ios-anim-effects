...
extension Effect {
    static let gesturesPhotoViewer = Effect(
        ...
        prompt: L(
            ...
            "300×340 pt 的风景照放在圆角视窗里。双指捏合时以两指中点为中心缩放，让那一点始终留在指下，超出 1×–4× 时带橡皮筋阻尼，松手弹回；放大后单指平移 1:1 跟手，甩出后沿方向继续滑行并指数减速（时间常数约 0.3 秒），拖到边缘会遇到橡皮筋阻力，再以弹簧（响应 0.45 秒、阻尼 0.85）回到画面内。双击以点击处为中心放大到 2.5×，或恢复适配，弹簧为响应 0.4 秒、阻尼 0.82，伴随轻触感，角落胶囊滚动显示倍率。照片在每个倍率下都重新绘制，路牌、小屋窗户、星星始终锐利。熟悉而精准。"
        ),
        implementation: L(
