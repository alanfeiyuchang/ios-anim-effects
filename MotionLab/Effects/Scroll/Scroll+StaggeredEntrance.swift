...
extension Effect {
    static let scrollStaggeredEntrance = Effect(
        ...
        prompt: L(
            "When a list of rounded rows loads, each row starts 28 pt below its resting position at 0% opacity, 96% scale and a 6 pt blur, then springs into place (response ≈0.55 s, damping 0.82) as it sharpens and fades up. Rows enter in reading order with a 60 ms stagger, so the content cascades down the screen like a deck being dealt rather than popping in at once; rows scrolled into view later run the same entrance. The staggering stops growing after the first screenful to keep long lists snappy. Calm, polished and confidence-inspiring.",
            "圆角列表加载时，每一行从静止位置下方 28 pt 处开始，透明度 0%、缩放 96%、模糊 6 pt，随后以弹簧（响应约 0.55 秒、阻尼 0.82）归位，同时变清晰并淡入。各行按阅读顺序错开 60 毫秒依次入场，内容像发牌一样沿屏幕向下倾泻，而不是一次性全部弹出；之后滚动进入视野的行也会执行同样的入场动画。错峰延迟在首屏之后不再累加，保证长列表依旧利落。沉稳、精致，令人信赖。"
        ),
        implementation: L(
            "Each row owns an @State `visible` flag flipped in onAppear inside withAnimation(.spring.delay(index × stagger)); bumping an .id on the ScrollView recreates the rows to replay the entrance.",
            "每一行持有自己的 @State `visible`，在 onAppear 中于 withAnimation(.spring.delay(序号 × 间隔)) 内切换；改变 ScrollView 的 .id 即可重建各行以重播入场。"
        ),
...
private struct ScrollStaggerDemo: View {
    ...
    var body: some View {
                ...
                LazyVStack(spacing: 10) {
                    ForEach(0..<14, id: \.self) { i in
                        ScrollStaggerRow(
                            ...
                            delay: Double(min(i, 6)) * ctx["stagger"],
        ...
        .autoplay(ctx.isPreview, every: 3.2) { generation += 1 }
    }
}
