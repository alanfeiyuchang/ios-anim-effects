...
extension Effect {
    static let iconsBookmarkSave = Effect(
        ...
        prompt: L(
            ...
            "文章卡片角落有一枚44×58 pt、3 pt描边的书签丝带。点收藏时，丝带以上边为锚点在110毫秒内被“扯”到125%高，再以约0.5秒的弹性关键帧经94%回到100%；同时琥珀到珊瑚的渐变色在350毫秒内自上而下灌满，缺口两侧各甩出一粒小火花，400毫秒内消散。旁边滑出一枚「已收藏」小胶囊，1.2秒后收回。取消收藏时丝带压扁到90%，颜色在250毫秒内向上退去；每次切换都有一下轻触感，像布料而不是扁平的图标替换。"
        ),
        implementation: L(
