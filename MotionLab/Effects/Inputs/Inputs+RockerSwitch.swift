...
extension Effect {
    static let inputsRockerSwitch = Effect(
        ...
        prompt: L(
            ...
            "设备面板上嵌着一枚硬件风翘板开关（132 × 72pt 按板，标有 O 与 I），四周是下凹边框。点击时外壳先压到 97%，按板再以强透视绕竖轴翻转 12°，弹簧极硬、接近临界阻尼（响应 0.18 秒、阻尼 0.9）——不晃，只有一声干脆的“咔嗒”与一次硬朗触觉。光影随倾角变化：翘起的一半出现高光渐变，按下的一半落入阴影，边框内阴影随之换边。随后指示灯闪烁点亮（260 毫秒内 0 → 100% → 40% → 100%），晕开琥珀光；关闭时瞬间熄灭。机械而厚重。"
        ),
        implementation: L(
