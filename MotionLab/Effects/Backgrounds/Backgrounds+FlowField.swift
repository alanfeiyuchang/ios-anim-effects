...
extension Effect {
    static let backgroundsFlowField = Effect(
        ...
        prompt: L(
            "On a near-black ink backdrop, ~260 particles trace an invisible vector field whose direction sums three slowly evolving sine terms, so the current bends into broad, ever-changing swirls. Each particle leaves a trail sampled every 1/30 s of simulated time (14 samples, ≈ 0.45 s of path), so its length depends on speed, never frame rate; it tapers in three segments from a faint, thin tail to a bright, thicker head, in one of four hues (indigo, violet, sky, mint) blended additively, so dense lanes glow brighter. Particles live 2.5–6 s and fade in and out, so nothing pops. Tapping or dragging sideways turns the finger into a vortex: within ≈ 120 pt particles gain a tangential swirl and a slight outward push, drawing a luminous whirlpool that dissolves back into the current on release. Generative, meditative, quietly premium.",
            "近黑的墨色背景上，约 260 个粒子沿看不见的向量场漂流：方向由三组缓慢演变的正弦项叠加而成，洋流弯成宽阔多变的漩涡。每个粒子的轨迹按模拟时间每 1/30 秒采样一次（14 个采样点，约 0.45 秒路径），长度只由速度决定、与帧率无关；轨迹分三段，从暗淡纤细的尾部渐变到明亮稍粗的头部，取靛蓝、紫、天蓝、薄荷之一，叠加混合，密集处更亮。粒子寿命 2.5–6 秒，生灭都有淡入淡出。点击或横向拖动时手指化身漩涡中心：约 120pt 内的粒子获得切向旋转与轻微外推，绘出发光涡流，松手后融回洋流。"
        ),
        implementation: L(
