...
extension Effect {
    static let scrollArcDial = Effect(
        ...
        prompt: L(
            "A horizontal row of 64 pt circular filter swatches is bent onto a large invisible wheel below the stage: as they scroll, each item is placed on the arc by its distance from centre — rotated ~15° per step, dropping and tilting away, scaling down ~10% per step and fading past three steps. A fixed hairline ring marks the centre; the item under it grows to 112% and snaps in with a firm deceleration, a selection tick and a crisp label cross-fade. The preview panel above cross-fades its gradient to the selected filter on a soft spring. Tactile and dial-like, as if turning a physical lens ring.",
            "一排 64 pt 的圆形滤镜色块被“弯”到舞台下方一个看不见的大转轮上：滚动时，每个选项依据与中心的距离被放置到弧线上——每隔一格旋转约 15°、向下沉并向外倾斜、缩小约 10%，超过三格后逐渐淡出。中心有一道固定的细线选择环；落入环中的选项放大到 112%，以干脆的减速吸附到位，并伴随一次选择触感与标签的清晰淡入淡出。上方预览面板的渐变以柔和弹簧过渡到所选滤镜。像拧动真实镜头环一样富有手感。"
        ),
        implementation: L(
