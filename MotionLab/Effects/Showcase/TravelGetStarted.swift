...
extension Effect {
    static let showcaseGetStarted = Effect(
        ...
        prompt: L(
            "An onboarding screen over a golden-hour mountain photo ends in a cream “Get Started” pill (56 pt tall) with a lime circular arrow badge. At rest, an orange highlight band sweeps across the label every 2.2 s, and the arrow nudges 6 pt to the right on a loop (ease-out out, springy return). On tap, the pill first squashes like jelly (x 108%, y 95%) over ~160 ms, then its background morphs via shared geometry into a full-width rounded sheet on a spring (response 0.5 s, damping 0.78). The title flies into the sheet header while the photo behind scales to 106%, blurs 6 pt and dims 15%. The sheet’s fields fade and rise 10 pt one after another at 50 ms intervals. A medium haptic confirms the press. It feels eager, elastic and welcoming.",
