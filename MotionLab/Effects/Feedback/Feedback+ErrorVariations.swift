...
extension Effect {
    static let feedbackLimitBounce = Effect(
        ...
        prompt: L(
            "A ticket stepper — minus button, rolling count, plus button in a 200 × 52 pt capsule — sits under 'Tickets · Max 4 per order'. Valid taps roll the digit with a numeric transition and a light tick. Tapping plus at the limit does not shake: the capsule stretches 6% toward the tapped side from its leading anchor in 0.1 s, the digit rises 10 pt and drops back past its line by 3 pt, and both snap home on a bouncy spring (digit 0.3 s, capsule 0.4 s) — a rubber band hitting its end stop. The plus glyph dims to 30%, the border flashes red and the 'Max 4' caption turns red and pulses to 106%, with a rigid haptic. Clear boundaries, zero scolding.",
            "一个购票步进器——减号、滚动数字、加号，装在 200 × 52 pt 的胶囊里——位于“票数 · 每单最多 4 张”下方。有效点击会让数字以数字转场滚动并伴随轻触感。在上限时点击加号不会抖动：胶囊以前缘为锚点在 0.1 秒内向点击一侧拉长 6%，数字上升 10 pt 再越过基线下沉 3 pt，然后二者以弹跳弹簧弹回原位（数字 0.3 秒、胶囊 0.4 秒）——像橡皮筋撞到尽头。加号变暗到 30%，边框闪红，“最多 4 张”说明文字变红并脉冲到 106%，同时伴随一次硬朗触感。边界清晰，从不责备。"
        ),
        implementation: L(
