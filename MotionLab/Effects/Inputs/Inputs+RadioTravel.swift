...
extension Effect {
    static let inputsRadioTravel = Effect(
        ...
        prompt: L(
            ...
            "配送方式列表共四行，每行有 22pt 单选圆环、标题、预计送达时间与价格。圆环里的实心圆点只有一个：选择另一行时，它会沿这一列圆环“游”过去。前缘先以快速弹簧（响应 0.28 秒、阻尼 0.8）出发，后缘晚 90 毫秒以更柔的弹簧（响应 0.42 秒、阻尼 0.7）跟上，于是先被拉成跨越整段距离的胶囊，再像水滴一样缩进新圆环。浅色圆角高光以匹配弹簧滑到所选行背后，圆环描边变靛蓝，底部总价滚动更新，落定时触发选择触觉。流畅连续，单选一目了然。"
        ),
        implementation: L(
