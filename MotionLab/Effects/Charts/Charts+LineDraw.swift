...
extension Effect {
    static let chartsLineDraw = Effect(
        ...
        prompt: L(
            ...
            "一张 290 × 170pt 的营收折线图，带四条标注 $8k～$26k 的淡色参考线与隔月的月份标签（1 月～9 月）：Catmull-Rom 平滑曲线（3pt，圆角端点，靛蓝 → 紫 → 粉渐变），下方面积从 35% 靛蓝渐隐到基线透明。出现与点击时，一条从左向右扫过的裁切边在 1.6 秒内揭示图表（ease-in-out 0.65, 0, 0.35, 1），曲线与面积共用同一裁切边。一个 10pt 白边圆点带柔和光晕，精确贴着裁切边处的曲线移动，并携带实时滚动的数值胶囊。重播时先在 250ms 内倒带收回，换入新数据后再次绘制。优雅而富有叙事感。"
        ),
        implementation: L(
