...
extension Effect {
    static let scrollRotaryWheel = Effect(
        ...
        prompt: L(
            "A 250 pt circular dial carries twelve 40 pt icon badges evenly around a 100 pt radius, over a faint tick ring. Scrolling vertically anywhere on the dial rotates it — one 30° notch per 56 pt of scroll — and the scroll snaps to whole notches, so an icon always comes to rest at 12 o'clock under a small triangular marker. The badge in that slot grows to 1.3× and gains a gradient ring and glow, while the others stay upright (they orbit without spinning) at 85% opacity. The dial's center shows the selected title, cross-fading on change, and every notch ticks a selection haptic. Tactile and mechanical, like an iPod click wheel or a camera mode dial.",
            "一个 250 pt 的圆形转盘上，十二个 40 pt 的图标徽章均匀分布在半径 100 pt 的圆周上，底下是一圈淡淡的刻度环。在转盘上任意位置上下滚动都会转动它——每滚动 56 pt 转过一个 30° 的档位——滚动会吸附到整档，因此总有一个图标停在 12 点方向的小三角标记下。处在该位置的徽章放大到 1.3 倍，并带上渐变光环与辉光，其余徽章保持正立（公转而不自转），透明度 85%。转盘中心显示选中项的标题，切换时交叉淡入淡出，每过一档都伴随选择触感。富有触感与机械感，就像 iPod 的点按式转盘或相机的模式拨盘。"
        ),
        implementation: L(
            "An invisible vertical ScrollView (clear content with a contentShape) sits on top of the dial and snaps to 56 pt steps; onScrollGeometryChange converts the offset into a continuous rotation that positions each badge with cos/sin, so icons orbit but never spin.",
            "一个不可见的纵向 ScrollView（透明内容加 contentShape）覆盖在转盘上方，并按 56 pt 吸附；onScrollGeometryChange 将偏移换算为连续的旋转角度，用 cos/sin 摆放每个徽章，因此图标只公转、不自转。"
        ),
...
private struct ScrollRotaryDemo: View {
    ...
    private var scroller: some View {
        ...
        return ScrollView {
            Color.clear
                .frame(height: dial + pitch * CGFloat(scrollRotaryCount - 1))
                .contentShape(Rectangle())
        }
        .scrollIndicators(.hidden)
        .scrollTargetBehavior(ScrollStrideSnap(stride: pitch, axis: .vertical))
    ...
    private func autoTurn() {
        ...
        withAnimation(.spring(response: 0.8, dampingFraction: 0.82)) {
            position.scrollTo(y: CGFloat(target) * pitch)
        }
    }
}
