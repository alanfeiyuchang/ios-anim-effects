...
extension Effect {
    static let gesturesPendulumSwing = Effect(
        ...
        prompt: L(
            "A 150 × 200 pt ID badge (22 pt continuous corners, elevated surface, violet header band, avatar and placeholder lines) is held by a lanyard hole at its top edge. Dragging moves the hole 1:1 while the badge pivots around it: horizontal finger velocity maps to a lag angle of up to ±28°, applied through an under-damped spring (response 0.45 s, damping 0.4), so the badge trails behind, swings past vertical when you stop and sways back. If the finger holds still for 90 ms the angle relaxes to zero. On release the badge flies home on a spring (response 0.5 s, damping 0.7) while an opposite kick of the return velocity makes it swing two or three times before hanging still, with a soft haptic. Weighty, physical and charming.",
            "一张 150 × 200pt 的工牌（22pt 连续圆角、浮起表面、紫色顶栏、头像与占位文字行）被顶部的挂绳孔“捏”住。拖动时挂孔 1:1 跟手，工牌绕挂孔摆动：手指的水平速度映射为最多 ±28° 的滞后角，经欠阻尼弹簧（响应 0.45 秒、阻尼 0.4）作用，于是工牌拖在手指后方，手指停下时会越过竖直位置再摆回。手指静止 90ms 后角度回归零度。松手后工牌以弹簧（响应 0.5 秒、阻尼 0.7）飞回原位，回程速度带来一个反向摆动，使它来回摆荡两三次后才静静垂下，并伴随柔和触感。有分量、有物理感，也很讨喜。"
        ),
        implementation: L(
