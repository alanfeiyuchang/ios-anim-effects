...
extension Effect {
    static let showcaseSleepTimeline = Effect(
        ...
        prompt: L(
            "A dark sleep widget: a “7h 42m” time-asleep headline that counts up, a small replay button, and a four-lane hypnogram (Awake, REM, Core, Deep) of rounded colored blocks joined by thin connectors, with 23:10 · 03:00 · 06:52 below and a legend of per-stage totals with mini bars. On appear a soft-edged reveal sweeps the chart left to right in ~1.4 s (ease-in-out) with a glowing leading edge, while the headline counts up in 14 eased steps and each legend bar grows from its leading edge 60 ms after the previous. Dragging across the chart drops a hairline cursor with a bubble reading the clock time and stage (“02:14 · Deep”); blocks of the other stages dim to ~40% on a spring (response 0.3 s, damping 0.8), the matching legend row brightens, and a selection haptic ticks each time the stage under the finger changes. Lifting the finger releases the spotlight. Calm, restorative and precise.",
            "深色睡眠小组件：会递增计数的“7 小时 42 分”睡眠时长标题、一个小小的重播按钮，以及分为清醒、快速眼动、核心、深睡四条泳道的睡眠阶段图——彩色圆角色块由细连接线串起，下方标有 23:10 · 03:00 · 06:52，再下面是各阶段时长与迷你进度条的图例。出现时，一道柔边揭示带着发光的前沿在约 1.4 秒内（缓入缓出）自左向右扫出整张图，标题以 14 个缓出步进递增，图例进度条从左边缘依次生长，每条间隔 60 毫秒。在图上横向拖动会出现一条细游标和气泡，显示时间与阶段（“02:14 · 深睡”）；其他阶段的色块以弹簧（响应 0.3 秒、阻尼 0.8）暗到约 40%，对应图例行被点亮，手指下的阶段每变化一次触发一次选择触感。松手后高亮解除。安静、舒缓、精确。"
        ),
        implementation: L(
