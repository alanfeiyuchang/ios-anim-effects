...
extension Effect {
    static let morphBlurReplace = Effect(
        ...
        prompt: L(
            "A Dynamic Island–style black capsule and a large status tile below it cycle through live states: AirPods connected, timer running, payment complete, focus on. On each change the outgoing icon and label blur out while scaling down and the incoming content resolves from a blur while scaling up into place (in the Up-Up style both scale up, so the old content swells away), overlapping into one continuous dissolve. Meanwhile the capsule's width springs to its new content's measured width (response 0.45 s, damping 0.8), clipping the dissolve inside it, the tile's SF Symbol swaps with the native replace effect and the accent color tints across. One living surface re-forming, not a slideshow.",
            "一枚灵动岛式黑色胶囊和下方的大号状态卡片，轮流展示实时状态：耳机已连接、计时进行中、支付完成、专注模式开启。每次切换，旧图标与文字边缩小边在模糊里散去，新内容从模糊中清晰起来、放大落位（“始终放大”样式下两者都放大，旧内容像膨胀着消散），两者重叠成一段连贯的溶解。胶囊宽度同时以弹簧（响应 0.45 秒、阻尼 0.8）贴合新内容的实测宽度，把溶解裁在胶囊里；卡片的 SF Symbol 用原生替换特效切换，强调色随之晕染过去。像同一块活的表面在重组，而不是翻幻灯片。"
        ),
        implementation: L(
