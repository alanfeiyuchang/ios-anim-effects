...
extension Effect {
    static let chartsSparklineStream = Effect(
        ...
        prompt: L(
            "Three stacked monitor rows (CPU, Memory, Network) in rounded cards, each with a caption, a live percentage in rounded tabular digits and a 54 pt-tall sparkline in its own hue (indigo, mint, coral) over a soft vertical gradient fill. New samples arrive every 0.4 s from a mean-reverting random walk, but the chart never jumps: the whole series scrolls left continuously at one sample-width per interval, and the newest point enters from beyond the right edge while the leading dot — a 3.5 pt core with a 7 pt translucent halo — stays pinned to the right edge, its height interpolated between the last two samples. The percentage label reads the same interpolated value so it glides rather than ticks. Calm, continuous, ‘always live’ — like Activity Monitor rendered by a motion designer.",
            "三行纵向排列的监控卡片（CPU、内存、网络），每行包含说明文字、以圆体等宽数字显示的实时百分比，以及一条高 54pt 的迷你折线，分别使用靛蓝、薄荷绿、珊瑚色，下方配柔和的竖向渐变填充。新样本每 0.4 秒由均值回归的随机游走产生，但图表从不跳动：整条序列以“每个间隔移动一个样本宽度”的速度持续左移，最新数据点从右边界外滑入；领头圆点（3.5pt 实心 + 7pt 半透明光晕）固定在右边缘，高度在最后两个样本之间插值。百分比读数使用同一插值结果，因此平滑滑动而非跳变。沉静、连续、始终在线——像由动效设计师重绘的活动监视器。"
        ),
        implementation: L(
