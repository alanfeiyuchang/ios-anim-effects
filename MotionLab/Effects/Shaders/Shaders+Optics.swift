...
extension Effect {
    ...
    static let shaderKaleidoscope = Effect(
        ...
        prompt: L(
            ...
            "280pt 的圆形视窗呈现由满版插画构成的万花筒：圆点、花瓣、短条与星芒分布在环绕整圈的三道圆环上，每片楔形都有内容。Metal 着色器把像素转到极坐标，将角度归入 N 个楔形之一（默认 8 个），以楔形中线为轴镜像折叠到 [0, π/N]，再以该角度、同一半径采样原图，一小片内容便无缝复制成镜像对称的花窗。两个旋转同时进行：采样楔形以约 0.35 rad/s 转动，如旋转镜筒般不断绽放重组；整朵花窗以约 0.15 rad/s 反向漂移。水平拖动直接扭转镜筒，柔和轮廓光与外发光为镜片收边。催眠而多变。"
        ),
        implementation: L(
