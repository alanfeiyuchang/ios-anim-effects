...
extension Effect {
    static let textScramble = Effect(
        ...
        prompt: L(
            "A monospaced status line starts as a field of rapidly cycling random characters tinted mint, as if a signal is being decrypted. About a quarter of the way in, letters begin locking into their final value strictly left to right; each locked glyph snaps to the primary text colour while the unresolved tail keeps flickering at ~20 changes per second. The whole line resolves in ~1.6 s, then a small status dot turns green with a success haptic. Technical, precise and a little cinematic — hacker-film energy with product-grade restraint.",
            "一行等宽状态文字起初是一片快速跳动的随机字符，染成薄荷色，如同信号正在被解密。进行到约四分之一时，字符开始严格按从左到右的顺序锁定为最终内容；每个锁定的字立即切换为主文字颜色，尚未解出的尾部仍以约每秒 20 次的频率闪烁。整行在约 1.6 秒内全部解出，随后状态圆点变绿并伴随成功触感。技术感强、精准且带一点电影氛围——黑客电影的气质，产品级的克制。"
        ),
        implementation: L(
