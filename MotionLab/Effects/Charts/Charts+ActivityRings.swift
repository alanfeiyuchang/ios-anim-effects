...
extension Effect {
    static let chartsActivityRings = Effect(
        ...
        prompt: L(
            ...
            "三条同心圆环（描边 22pt、间距 4pt、外径 210pt）依次为“活动”红粉、“锻炼”青柠、“站立”青蓝，下方各有 20% 透明度的同色轨道，12 点处嵌粗体小图标。出现与每次点击时，圆环先在 250ms 内清空，再从顶部顺时针填充，由外向内错开 150ms，采用平滑弹簧（响应约 1.2 秒、阻尼 0.82）。弧线为角向渐变，起点深、末端亮，起点端帽为纯色、不留接缝。超过 100% 时圆环继续前进：渐变随之旋转，亮端始终在末端，端帽投下 3pt 柔影（自 85% 起渐显），如真实表带般重叠。百分比同步递增。"
        ),
        implementation: L(
