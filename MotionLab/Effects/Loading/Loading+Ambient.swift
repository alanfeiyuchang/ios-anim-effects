...
extension Effect {
    static let loadingAudioWave = Effect(
        ...
        prompt: L(
            "A row of 13 rounded 6 pt bars with 5 pt gaps, filled by one continuous vertical gradient (violet → sky → mint) that spans the whole group. Each bar's height is driven by two layered sine waves at different frequencies and phase offsets, shaped by a bell-curve envelope so the center bars swing tallest and the edges stay calm — reading as organic, live audio rather than a mechanical equalizer. Bars grow symmetrically from the center line, or optionally from the baseline, under a 'Listening…' caption.",
            "一排 13 根 6 pt 宽的圆角竖条，间距 5 pt，整组共用一条连续的竖向渐变（紫罗兰 → 天蓝 → 薄荷绿）。每根竖条的高度由两道频率、相位各不相同的正弦波叠加驱动，再乘以钟形包络：中间的竖条摆幅最大，两侧较为平静——看起来像真实的实时语音，而非机械的均衡器。竖条默认从中线向上下对称伸缩，也可切换为从底线生长，下方配“正在聆听…”说明。"
        ),
        implementation: L(
            "A TimelineView computes each bar height; the bars form a mask over a single LinearGradient so the color spans the group.",
            "TimelineView 计算每根竖条高度，竖条作为遮罩覆盖在同一条线性渐变上，使颜色贯穿整组。"
        ),
        apis: ["TimelineView", "mask(alignment:_:)", "LinearGradient", "Capsule"],
        tags: ["audio", "waveform", "voice", "equalizer", "音频", "波形", "语音", "均衡器"],
        params: [
            .slider("count", L("Bars", "竖条数"), 5...21, default: 13, step: 1, decimals: 0),
            .slider("speed", L("Speed", "速度"), 0.3...2.0, default: 1.0),
            .slider("height", L("Max height", "最大高度"), 30...110, default: 72, decimals: 0, unit: "pt"),
...
private struct AudioWaveDemo: View {
    ...
    var body: some View {
        VStack(spacing: 20) {
            ...
            Text(ctx.language == .zh ? "正在聆听…" : "Listening…")
...
extension Effect {
    static let loadingPulseRings = Effect(
        ...
        prompt: L(
            "A 72 pt gradient core disc with a white antenna glyph and a soft colored shadow sits at the center. Three rings are emitted from behind it in an even stagger across a 2.2 s period; each expands from the core's edge to about 2.6× its size on a quadratic ease-out, its tinted fill fading from 18% to 0 and its 1.5 pt hairline stroke from 60% to 0 as it grows. The core itself breathes subtly (±3%). Continuous and calm, it communicates 'searching nearby' like AirDrop or Find My.",
            "中心是一枚 72 pt 的渐变圆形核心，内含白色天线图标并带柔和彩色投影。三圈圆环在 2.2 秒周期内均匀错峰地从核心背后发出：每圈以二次缓出从核心边缘扩大到约 2.6 倍，扩散时着色填充从 18% 淡到 0、1.5 pt 细描边从 60% 淡到 0。核心本身轻微呼吸（±3%）。持续而平静，传达出类似隔空投送或“查找”的“正在搜索附近”。"
        ),
        implementation: L(
            "A TimelineView derives each ring's normalized age from a shared clock plus index offset and maps it to frame size and opacity.",
            "TimelineView 由共享时钟加索引偏移得到每圈的归一化“年龄”，再映射为尺寸与透明度。"
        ),
...
extension Effect {
    static let loadingSquareGrid = Effect(
        ...
        prompt: L(
            "A 3×3 grid of 26 pt tiles with 8 pt gutters, tinted by one mint → sky → violet gradient spanning the whole grid. Each tile continuously morphs between a full rounded square (6 pt corners, 100% scale, 0°) and a small circle (45% scale, rotated 90°) on a cosine wave with a 1.6 s period. The phase is delayed 120 ms per diagonal step (row + column), so a soft ripple rolls from the top-left corner to the bottom-right. The corner radius, scale and rotation interpolate together, making it feel like a single breathing material.",
            "3×3 方格，每格 26 pt、间距 8 pt，整组共用一条薄荷绿 → 天蓝 → 紫罗兰渐变着色。每个方格沿 1.6 秒周期的余弦波，在完整圆角方块（6 pt 圆角、100% 大小、0°）与小圆点（45% 大小、旋转 90°）之间连续形变。相位按对角线步数（行 + 列）每步延迟 120 毫秒，于是一道柔和涟漪从左上角滚向右下角。圆角、缩放与旋转同步插值，仿佛整块材质在一起呼吸。"
        ),
        implementation: L(
