...
extension Effect {
    static let buttonsOrbitActions = Effect(
        ...
        prompt: L(
            "A 68 pt glowing violet core button sits at the centre of the stage. On tap it spins its sparkle glyph 180° while five 48 pt satellite actions spiral out onto a full 92 pt orbit: each travels on a polar path — its radius grows from 0 while its angle sweeps 140° into place — so they swirl out like a galaxy rather than sliding straight, 40 ms apart, on a spring (response 0.5 s, damping 0.72), and each stays upright while it travels. A dashed orbit ring draws itself behind them (trim 0 → 1 in 0.5 s). Satellites wear their own tint; tapping one pulses it and collapses the orbit back into the core in reverse, with a success haptic; tapping the core toggles with a light one. Cosmic, playful and memorable.",
            "舞台中央有一枚 68pt 的发光紫罗兰核心按钮。点击后，核心的闪光图标旋转 180°，五个 48pt 的卫星操作沿极坐标轨迹螺旋飞出，落到一整圈半径 92pt 的轨道上：每个卫星的半径从 0 增长，同时角度扫过 140° 就位，因此它们像星系一样旋出，而不是直线滑出；彼此错开 40 毫秒，使用弹簧（响应 0.5 秒、阻尼 0.72），飞行中始终保持正立。身后一圈虚线轨道随之绘制（trim 0 → 1，0.5 秒）。每个卫星有自己的色调；点击某个卫星会让它脉冲一下，并让整条轨道反向收回核心，伴随成功触感；点击核心则以轻触感切换。充满宇宙感、俏皮且令人印象深刻。"
        ),
        implementation: L(
