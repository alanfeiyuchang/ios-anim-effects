...
private let cubePages: [CubePage] = [
    CubePage(symbol: "sun.max.fill", colors: [Palette.amber, Palette.coral], title: L("Morning", "清晨")),
    CubePage(symbol: "leaf.fill", colors: [Palette.mint, Palette.sky], title: L("Garden", "花园")),
    CubePage(symbol: "sparkles", colors: [Palette.violet, Palette.pink], title: L("Studio", "工作室")),
    CubePage(symbol: "moon.stars.fill", colors: [Palette.indigo, Color(hex: 0x241B5C)], title: L("Night", "夜晚")),
]
...
private struct CubeTransitionDemo: View {
    ...
    var body: some View {
        ...
        .autoplay(ctx.isPreview, every: 1.6) {
            let next = progress.rounded() + 1
            snap(to: next > lastIndex ? 0 : next)
        }
    }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            Text(page.title, language)
                ...
                .foregroundStyle(.white)
            Capsule().fill(.white.opacity(0.35)).frame(width: 120, height: 8)
        }
        .padding(22)
