...
extension Effect {
    static let cardsTileFlip = Effect(
        ...
        prompt: L(
            "A 252×176 pt card is sliced into a 4×3 mosaic of 60×56 pt tiles with 4 pt gaps and 8 pt corners; together the tiles show a night scene. Tapping flips every tile 180° around its vertical axis in perspective, each on its own spring (response 0.55 s, damping 0.72), but started in a wave: tiles are delayed 50 ms per step along the diagonal from the top-left corner (or outward from the center, or in a scattered order). Mid-turn each tile shrinks to about 88%, widening the gaps, and its back face carries the matching slice of a daytime scene, so the new picture assembles as the wave passes. Tapping again plays the wave back. Crisp, rhythmic and satisfying, like a split-flap board.",
            "一张 252×176 pt 的卡片被切成 4×3 的马赛克，每块 60×56 pt，间隙 4 pt，圆角 8 pt，拼在一起是一幅夜景。点击后，每块瓷砖都绕竖直轴以透视翻转 180°，各自使用弹簧（响应 0.55 秒、阻尼 0.72），但按波浪依次启动：从左上角沿对角线每一步延迟 50 毫秒（也可从中心向外扩散，或随机错落）。翻到一半时瓷砖缩小到约 88%，间隙随之变宽；背面是白昼画面中对应的那一块，于是新画面随着波浪逐渐拼合。再次点击则反向翻回。清脆、有节奏、令人满足，就像翻牌显示屏。"
        ),
        implementation: L(
