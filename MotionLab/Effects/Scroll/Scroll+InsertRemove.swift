...
extension Effect {
    static let scrollInsertRemove = Effect(
        ...
        prompt: L(
            ...
            "收件箱式的圆角列表上方是标题栏，带实时计数和一个圆形渐变「+」按钮。点+会在顶部插入一行：新行从上方落下并淡入，已有各行以同一个弹簧（响应0.45秒、阻尼0.8）向下让位，整张列表像一块有弹性的整体。点某行的×时，它向右滑出并淡去，下方各行随即上移补位；也可以换成从70%缩放弹出，或经10 pt模糊溶解消失。计数以滚动数字过渡跳变，每次操作都有一下轻触感，因果清楚，毫无跳变。"
        ),
        implementation: L(
