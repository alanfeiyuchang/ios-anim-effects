...
extension Effect {
    static let cardsFlip = Effect(
        ...
        params: [
            .choice("axis", L("Flip axis", "翻转轴"), [L("Horizontal", "水平"), L("Vertical", "垂直")]),
...
private struct CardsFlipDemo: View {
    ...
    var body: some View {
        VStack(spacing: 28) {
            CardsFlipCard(angle: angle, vertical: ctx.int("axis") == 1, lift: ctx["lift"])
...
private struct CardsFlipBack: View {
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            HStack {
                PlaceholderLines(count: 2, color: .white.opacity(0.25))
                    .frame(width: 120)
                Spacer(minLength: 0)
                Circle()
