...
extension Effect {
    static let scrollCollapsingHeader = Effect(
        ...
        prompt: L(
            ...
            "页面顶部是一个 150 pt 的展开式标题区：64 pt 渐变头像、30 pt 粗体大标题与次级副标题。内容上滑时，标题区在前 90 pt 的滚动内被连续驱动：高度收缩到 60 pt，头像缩小到 30 pt，标题缩小到约 57% 并向右上滑动到头像旁边，副标题以两倍于滚动的速度淡出。与此同时，磨砂材质背景与细分隔线在下方淡入，让从下面经过的列表行依旧清晰可读。向下滚动时所有变化精确反向；若在折叠中途松手，列表会用 0.35 秒平滑停靠到更近的完全展开或完全折叠状态。标题区本身不依赖任何计时器，纯几何驱动，因此如同黏在指尖，像原生 iOS 导航栏，却更有个性。"
        ),
        implementation: L(
