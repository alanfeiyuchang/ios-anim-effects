...
extension Effect {
    static let inputsPasscodePad = Effect(
        ...
        prompt: L(
            ...
            "锁屏风格密码键盘：锁形图标与标题下方是四个 13pt 描边圆点，再往下是 3 × 4 的 54pt 圆形按键，大号数字配细小字母。触碰按键时底色瞬间提亮到 26%，松手后 350 毫秒缓出淡回，像背光玻璃，并轻触一下。每输入一位，下一个圆点以弹性弹簧（响应 0.3 秒、阻尼 0.55）从 85% 弹入。输满四位：错误则圆点变红，整行以衰减关键帧水平抖动（±14pt → 0，约 450 毫秒）并触发错误触觉后清空；正确则圆点变绿，锁经符号替换弹开并触发成功触觉。"
        ),
        implementation: L(
