...
extension Effect {
    static let chartsStackedBars = Effect(
        ...
        prompt: L(
            ...
            "一张“各渠道营收”卡片：六个月（4 月～9 月）的堆叠柱（柱宽 58%，圆角 3pt）由三个系列组成——订阅（靛蓝）、应用内购（粉）与广告（琥珀），背景为虚线网格，右侧为 $0～$25k 刻度。出现时各月柱子依次从基线升起，间隔 60ms，使用弹簧（响应约 0.55 秒、阻尼约 0.75）。下方三枚图例胶囊即是开关：关闭某系列时其分段缩为零、上方分段下滑补位，同在一个弹簧中完成，胶囊变暗并加删除线；重新打开则再次堆叠。标题总额以数字滚动更新为可见系列之和。点击图表会重播升起动画。清爽、易于比较与探索。"
        ),
        implementation: L(
