...
extension Effect {
    static let chartsBarRace = Effect(
        ...
        prompt: L(
            "A ranked horizontal bar chart of six rows (26 pt tall, 8 pt gaps): rank number, label, a 150 pt capsule track with a gradient bar in each item’s colour, and a counting value. Every refresh assigns new values and re-sorts descending in a single spring (response ≈ 0.6 s, damping ≈ 0.8): bars stretch or shrink to their new widths while the rows physically slide past each other to their new positions, so overtakes are visible rather than instant swaps. Rank digits roll with a numeric content transition, values count through intermediate numbers in sync with the bar widths, and the leader’s rank badge fills with its colour. Motion is simultaneous but never chaotic because every property shares one spring. It tells the story of change, not just the result.",
