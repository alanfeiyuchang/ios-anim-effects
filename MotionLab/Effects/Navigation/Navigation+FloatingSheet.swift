...
extension Effect {
    static let navigationFloatingSheet = Effect(
        ...
        prompt: L(
            "Over a map, a compact sheet floats 12 pt above the bottom and in from the sides as a glass card with 30 pt corners. Dragging it up (or tapping) moves through three detents — compact 96 pt, medium 190 pt, full — and every property is a function of height: side and bottom margins shrink from 12 pt to 0, the bottom corners flatten while the top corners ease from 30 to 24 pt, and the material shifts from translucent glass to an opaque surface so text becomes fully legible. The sheet tracks the finger 1:1 with rubber-banding past the ends, and on release springs (response ≈0.45 s, damping 0.8) to the detent nearest the drag's projected end, with a light tick per detent. It feels like the iOS 26 floating sheet settling into the device.",
            "地图上方，一块紧凑面板以玻璃卡片的形态悬浮在距底部与两侧 12pt 处，圆角 30pt。向上拖动（或点击）会经过三个档位——紧凑 96pt、中等 190pt、全屏——而且所有属性都是高度的函数：左右与底部边距从 12pt 缩小到 0，底部圆角逐渐变平、顶部圆角从 30pt 缓和到 24pt，材质从半透明玻璃过渡到不透明表面，让文字完全清晰。面板 1:1 跟手，超出两端时有橡皮筋阻尼；松手后依据拖拽的预测终点以弹簧（响应约 0.45 秒、阻尼 0.8）吸附到最近的档位，每到一个档位轻触一次。就像 iOS 26 的悬浮面板落定到设备边缘。"
        ),
        implementation: L(
