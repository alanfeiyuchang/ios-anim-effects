...
extension Effect {
    static let gesturesNewtonsCradle = Effect(
        ...
        prompt: L(
            ...
            "五颗 36 pt 的铬球（由白色高光经银色过渡到石墨色的径向渐变）以 140 pt 细绳并排挂在横梁下，地面各有一团柔和阴影，球升得越高影子越小。每颗球都是真实的单摆（g ≈ 2000 pt/s²，周期约 1.7 秒，轻微空气阻力），以 240 Hz 积分；拖动任一颗会让它沿绳弧摆起，最大 ±52°，松手时继承手指的切向速度。碰撞按近乎完全弹性（恢复系数 0.97）交替求解，动量几毫秒内穿过静止的球，最远端的球飞出、中间几乎不动；拉起两颗就弹出两颗。强碰撞时一下清脆触感，能量缓缓耗散，直到整排静止。"
        ),
        implementation: L(
