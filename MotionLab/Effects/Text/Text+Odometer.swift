...
extension Effect {
    static let textOdometer = Effect(
        ...
        prompt: L(
            ...
            "机械里程表以六个深色圆柱滚轮显示 4231.8 公里——五个整公里滚轮、一个琥珀色小数点和一个琥珀色的十分位滚轮。每个滚轮是一条无限循环的 0–9 数字带，只露出一个窗口，上下内阴影模拟圆柱曲面。读数增加时，各滚轮只会向前滚动（9 之后接着滚到 0，绝不倒转经过 8…1），以轻度欠阻尼弹簧（响应约 0.8 秒、阻尼 0.78）停到新数字，长距离滚动时阻尼自动趋近临界，回弹不超过半格，并从右向左依次错开约 60 毫秒，进位像真实齿轮逐级咬合般传递；十分位滚轮转得最快。每次滚动伴随轻微冲击触感——触感真实、复古而令人满足。"
        ),
        implementation: L(
