...
extension Effect {
    static let showcaseTripChips = Effect(
        ...
        prompt: L(
            ...
            "暗色“轻松规划旅程”卡片上方是一排分段筛选标签（全部 · 步道 · 夜景 · 湖泊），下面是三列照片网格。点击标签，唯一一枚带淡淡辉光的青柠色胶囊借共享几何以弹簧（响应 0.42 秒、阻尼 0.78）滑到新标签背后，文字由浅灰白变黑。不再匹配的卡片在原位 150 毫秒内缩小消失；60 毫秒后保留的卡片滑到新格位，新卡片在各自格位 100 毫秒后从 60% 缩放淡入，每张晚 45 毫秒，全程互不重叠。计数滚动更新，每次切换一次选择触感。流畅有序。"
        ),
        implementation: L(
