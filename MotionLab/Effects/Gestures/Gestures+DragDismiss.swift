...
extension Effect {
    static let gesturesDragDismiss = Effect(
        ...
        prompt: L(
            ...
            "一张 220×260 pt 的详情卡片（渐变主图、标题与文字行、14 pt 圆角、深投影）悬浮在变暗的缩略图网格之上。向下拖动时卡片跟手，并以按住的点为锚点从 100% 交互式缩向 70%，让那一点始终留在指下；圆角由 14 pt 增到 44 pt，横向位移衰减为 60%，背景遮罩逐渐褪去，卡片看起来正“变回”缩略图；向上拖则带橡皮筋阻尼。松手时若下拉超过 140 pt 或预测终点超过 320 pt，卡片缩到 35% 并下坠淡出（弹簧响应 0.45 秒、阻尼 0.85），否则弹回全尺寸。直接可逆，就像 iOS 里下拉关闭照片。"
        ),
        implementation: L(
