...
private struct MarqueeDemo: View {
    ...
    var body: some View {
        VStack(spacing: 22) {
            ...
            DemoHint(text: L("Drag a belt to scrub or fling it", "拖动传送带来回拨动或甩出"), ctx: ctx)
        }
...
private struct MarqueeRow<Content: View>: View {
    ...
    private func release(velocity: CGFloat) {
        guard let start = heldAt else { return }
        let now = Date()
        manual += liveX
        liveX = 0
    ...
    /// Distance coasted since the last fling: v·τ·(1 − e^(−t/τ)).
    private func flingOffset(at date: Date) -> CGFloat {
        let t = max(date.timeIntervalSince(flingStart), 0)
        return flingVelocity * CGFloat(coast * (1 - exp(-t / coast)))
    }
    ...
    private func offset(at date: Date) -> CGFloat {
        ...
        return CGFloat(wrapped > 0 ? wrapped - width : wrapped)
    }
