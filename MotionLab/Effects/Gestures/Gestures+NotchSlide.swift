...
extension Effect {
    static let gesturesNotchSlide = Effect(
        ...
        prompt: L(
            ...
            "一条290×64 pt的胶囊滑轨被3 pt间隙分成四段，上方是步骤标签（核对·验证·授权·支付），左侧是带银行卡图标的56 pt白色滑块。拖动在每个刻口附近带磁性：距刻口18 pt以内，滑块偏移按二次曲线缓动，像被吸住再追上手指，每过一个刻口都有硬朗的“咔哒”触感。越过的分段以挤压弹跳（响应0.3秒、阻尼0.5）填成靛蓝，对应标签变粗。拖到终点即提交：各段从左到右间隔60毫秒依次变绿，图标换成对勾，伴随成功触感；中途松手则弹回，分段从右到左熄灭。干脆可靠。"
        ),
        implementation: L(
