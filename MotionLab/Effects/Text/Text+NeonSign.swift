...
extension Effect {
    static let textNeonSign = Effect(
        ...
        prompt: L(
            "A dark sign panel carries two neon lines: a large pink word and a smaller sky-blue sub-line, each letter a pale tube core wrapped in three stacked glows (4, 12 and 28 pt). Unlit tubes remain faintly visible at 18%. On power-on every letter runs the same stutter — off 80 ms, flash to 90%, drop to 10%, full, sag to 25%, 80%, 15%, then steady at 100% after ~0.46 s — but letters start 70 ms apart, so ignition crackles across the word, and the sub-line follows 250 ms later. Once lit, one faulty letter keeps dipping for a few frames every ~1.4 s. A soft bloom on the panel brightens with the tubes. A tap cuts the power and restarts it with a rigid haptic. Moody, nocturnal, cinematic.",
            "一块深色灯牌上有两行霓虹字：上方是大号粉色单词，下方是较小的天蓝色副标题，每个字母都是浅色灯芯外裹三层叠加辉光（4、12、28 pt）。未点亮的灯管保留 18% 的微弱可见度。通电时每个字母都经历同样的抽动序列——先灭 80 毫秒，闪到 90%，跌到 10%，全亮，再塌到 25%、80%、15%，约 0.46 秒后稳定在 100%——但字母之间错开 70 毫秒，于是点火的噼啪声沿着单词传过去；副标题再晚 250 毫秒。点亮后，其中一个坏掉的字母每隔约 1.4 秒会闪暗几帧。灯牌背板上的柔光随灯管一起变亮。点击会断电并重新通电，伴随清脆触感。氛围感十足，夜色、电影感。"
        ),
        implementation: L(
