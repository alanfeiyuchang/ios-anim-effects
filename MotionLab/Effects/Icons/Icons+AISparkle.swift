...
extension Effect {
    static let iconsAISparkle = Effect(
        ...
        prompt: L(
            ...
            "如今常见的「AI」标志：一个四边内凹、呈弧形的四角大星芒，填充由紫、粉、琥珀、天蓝组成的锥形渐变，渐变每秒旋转 60°，颜色仿佛绕着图形流动。星芒以 4 秒周期的正弦做 ±12° 摇摆，并在 92% 到 106% 之间呼吸缩放，下方衬着一层同样渐变、50% 透明度的模糊光晕。两颗小卫星星芒（右上与左下）反相闪烁：每 1.6 秒从 30% 放大到 100% 并旋转 45°，再缩回。切到「思考中」时，所有节奏加快 2.5 倍，并有一粒小火花每 1.2 秒绕星芒转一圈。聪明、温柔、有生命力。"
        ),
        implementation: L(
