...
extension Effect {
    static let backgroundsStarfieldWarp = Effect(
        ...
        prompt: L(
            "A deep-space backdrop — near-black fading to indigo at the center — filled with a few hundred stars projected in perspective from a central vanishing point. Each star travels along its own radial ray; its distance, brightness and line width grow as it approaches, and it is drawn as a streak whose length is proportional to current velocity, with a gradient that fades from transparent at the tail to full brightness at the head, and a subtle cyan or violet tint on a minority of stars. Pressing and holding still for 150 ms (so a page scroll never triggers it) eases velocity up to about 7× over ~0.5 s (exponential approach), stretching stars into long light-speed trails and blooming a soft blue glow at the core; releasing eases back to a leisurely cruise with no discontinuity. A medium haptic marks the engage. It feels immersive, kinetic and cinematic.",
            "深空背景由近乎纯黑向中心的靛蓝渐变，数百颗星星以中心灭点做透视投影。每颗星沿各自的径向射线飞来，越靠近则越亮、越粗，并以与当前速度成正比的拖尾线段绘制，拖尾由尾端透明渐变到头部全亮，少数星星带有青色或紫色色调。按住不动 150 毫秒后（页面滚动不会误触发）速度以指数趋近方式在约 0.5 秒内加速到约 7 倍，星点被拉成长长的光速轨迹，中心泛起柔和蓝色辉光；松手后平滑减速回巡航状态，全程无跳变。按下时伴随中等强度的触觉反馈。整体沉浸、充满速度感与电影感。"
        ),
        implementation: L(
...
private struct WarpDemo: View {
    ...
    /// Bumped on every press change, so a pending engage only fires if the finger is still down and still.
    ...
    var body: some View {
        ...
        // pressing turns true at touch-down, so the boost and haptic wait for a 150 ms still hold:
        // a page scroll that starts on the stage moves 10 pt (failing the press) before it engages.
        .onLongPressGesture(minimumDuration: 60, maximumDistance: 10, perform: {}, onPressingChanged: { pressing in
            ...
            Task { @MainActor in
                ...
                guard token == pressToken, !model.boosting else { return }
                model.boosting = true
                Haptics.tap(.medium)
            }
        })
