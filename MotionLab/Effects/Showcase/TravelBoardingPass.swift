...
extension Effect {
    static let showcaseBoardingPass = Effect(
        ...
        prompt: L(
            "A cream 270 pt boarding pass on a dark stage: a main panel with HGH → NCE and flight details over a 74 pt stub, split by a dashed perforation with half-circle notches in both edges. Dragging the stub down makes it follow at 85% of the drag, hinging from its top-left corner by up to ~6° with a slight sideways drift; a rigid haptic clicks as the pull passes the 90 pt tear threshold. Released past it, the stub tears free and falls 420 pt on an ease-in gravity curve over 600 ms while spinning ~22°, with a success haptic; 1.3 s later a fresh stub springs in from 90%. Released short, it snaps back on a bouncy spring. Tapping the main panel flips the whole ticket 180° in 3D over 0.55 s, faces swapping exactly halfway to reveal a barcode and the stub's back. Physical and crisp.",
            "暗色舞台上一张 270pt 宽的奶油白登机牌：主票面写着 HGH → NCE，下方 74pt 票根，中间是带半圆缺口的虚线齿孔。向下拖票根，它以 85% 距离跟手，以左上角为铰点最多倾斜约 6° 并略微横漂；拉过 90pt 阈值时硬朗地“咔”一下。过阈值松手，票根被撕下，以缓入重力曲线 600 毫秒坠落 420pt、旋转约 22°，并触发成功触感，1.3 秒后新票根从 90% 弹回；未过阈值则弹性复位。点击主票面，整张票 0.55 秒内立体翻转 180°，露出条形码。"
        ),
        implementation: L(
