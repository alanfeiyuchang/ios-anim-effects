...
extension Effect {
    static let gesturesChargeBurst = Effect(
        ...
        prompt: L(
            "A 112 pt circular core (indigo-to-violet gradient, bolt glyph) sits inside a 168 pt track ring. Pressing and holding fills the ring clockwise from 12 o’clock with a mint → sky → violet → pink angular gradient over a linear 1.2 s, while the core sinks to 90% and its coloured glow grows; a percentage counts up beneath in tabular digits. At 100% a heavy haptic fires and the core trembles with a fast 1.5 pt jitter to signal it is primed. Releasing early drains the ring on a smooth spring; releasing when full triggers a burst: 14 capsule particles fly outward 60 → 170 pt while shrinking and fading, a shock-wave ring expands and dissolves over 0.8 s, and the core pops past 100% on a bouncy spring (damping 0.45). Suspense, then payoff.",
            "一个 112pt 的圆形核心（靛蓝到紫色渐变、闪电图标）位于 168pt 的轨道圆环中。长按时，圆环以薄荷绿 → 天蓝 → 紫 → 粉的角向渐变从 12 点方向顺时针线性填充（1.2 秒），同时核心下沉至 90%、彩色光晕逐渐增强，下方百分比以等宽数字同步递增。满格瞬间触发重触感，核心以 1.5pt 的高频抖动示意“已蓄满”。提前松手，圆环以平滑弹簧回落；满格松手则爆发：14 枚胶囊粒子由 60pt 飞散至 170pt 并逐渐缩小淡出，一圈冲击波在 0.8 秒内扩散消散，核心以高弹性弹簧（阻尼 0.45）回弹并略微超过原尺寸。先蓄势，后释放，张力十足。"
        ),
        implementation: L(
...
private struct ChargeBurstDemo: View {
    ...
    var body: some View {
        VStack(spacing: 22) {
            ZStack {
                if burstCount > 0 {
                    BurstView(count: ctx.int("particles"))
                        .id(burstCount)
                }
...
private struct BurstView: View {
    ...
    private func particle(_ index: Int) -> some View {
        ...
        let radius = 60 + 110 * progress
        let color = Palette.spectrum[index % Palette.spectrum.count]
        return Capsule()
            .fill(color)
            .frame(width: 4 + 12 * (1 - progress), height: 5)
