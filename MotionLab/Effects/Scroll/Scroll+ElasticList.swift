...
extension Effect {
    static let scrollElasticList = Effect(
        ...
        prompt: L(
            ...
            "一段聊天记录，胶囊形消息气泡排在44 pt高的行中左右交替（收到的在左、发出的蓝色渐变在右），间距10 pt。气泡并没有和滚动牢牢粘在一起：每个气泡都通过自己的弹簧跟随，离运动前沿越远，弹簧越松（响应0.2→0.45秒、阻尼0.62），滞后也越多——滞后量为滚动速度（按时间戳换算为pt/s）×24毫秒，上限40 pt，60 Hz与120 Hz屏幕观感一致。因此快速滚动时气泡间距像手风琴一样被拉开，滚动停下后，气泡带着轻微过冲弹回聚拢。布局不变，只有偏移在动。灵动真实，一眼就是iOS。"
        ),
        implementation: L(
