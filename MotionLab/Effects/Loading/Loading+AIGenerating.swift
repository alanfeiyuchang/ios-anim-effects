...
extension Effect {
    static let loadingAIGenerating = Effect(
        ...
        prompt: L(
            "A 290 pt reply card with 24 pt continuous corners. While the model thinks, a 2 pt angular-gradient border (sky → violet → pink → amber → mint) rotates around it once every 3 s, with a blurred 5 pt copy glowing outside the edge. Inside, a pulsing sparkle badge sits beside 'Writing a reply…', and four skeleton lines carry a violet-to-pink sheen sweeping left to right every 1.6 s. When the text is ready, border, glow and lines fade on a 0.4 s ease-out while the content settles on a spring (0.55 s, damping 0.85): the paragraph rises 6 pt out of an 8 pt blur, the title becomes 'Draft ready', a green check pops in and a success haptic fires. Magical but restrained.",
            "一张 290 pt 宽、24 pt 连续圆角的回复卡片。模型思考时，2 pt 的角向渐变描边（天蓝 → 紫罗兰 → 粉 → 琥珀 → 薄荷）每 3 秒绕卡片转一圈，5 pt 的模糊副本在边缘外晕出柔光。卡内，脉动的星光徽标旁写着“正在撰写回复…”，四条骨架行上一道紫粉流光每 1.6 秒自左向右扫过。文字就绪时，描边、辉光与骨架以 0.4 秒缓出淡去，内容按弹簧（0.55 秒、阻尼 0.85）落定：正文从 8 pt 模糊中上浮 6 pt，标题换成“草稿已生成”，绿色对勾弹出，伴随成功触感。神奇而克制。"
        ),
        implementation: L(
