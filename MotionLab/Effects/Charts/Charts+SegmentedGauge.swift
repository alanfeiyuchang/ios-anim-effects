...
extension Effect {
    static let chartsSegmentedGauge = Effect(
        ...
        prompt: L(
            "A 230 pt gauge made of 30 capsule ticks (18 × 6 pt) spaced along a 240° arc open at the bottom, with the reading in the centre. Unlit ticks are 8% grey at 80% size. When the value rises, ticks switch on strictly one at a time, 25 ms apart, each popping to full size with a coloured glow on a quick ease-out (150 ms); the colour ramps from mint through amber (60%) to red at the top end. When the value falls, ticks switch off in reverse from the tip, at the same cadence. The centre number counts linearly for exactly as long as the ticks take and the status word (Normal / Busy / Critical) cross-fades to its colour. Hardware-like, rhythmic and readable.",
            "一个 230pt 的仪表，由 30 个胶囊刻度（18 × 6pt）沿底部开口的 240° 弧线排列，读数位于中心。未点亮的刻度为 8% 灰色、尺寸 80%。数值上升时，刻度严格逐格点亮，间隔 25ms，每格以 150ms 的快速缓出放大到原尺寸并带同色光晕；颜色从薄荷绿经琥珀色（60% 处）渐变到末端的红色。数值下降时，刻度从末端反向逐格熄灭，节奏相同。中心数字以线性方式计数，时长恰好与刻度动画一致，状态文字（正常 / 繁忙 / 过载）交叉淡入对应颜色。硬件质感、节奏分明、清晰易读。"
        ),
        implementation: L(
            "Each tick carries .animation(.easeOut.delay(d), value: isLit), where d is computed from its distance to the previous lit count in the direction of change; an Animatable text view counts with a linear animation whose duration matches the cascade.",
...
private struct SegmentedGaugeDemo: View {
    ...
    private func setRandom(haptic: Bool) {
        ...
        if abs(next - value) < 15 { next = value > 50 ? next - 40 : next + 40 }
        next = next.clamped(to: 4...99)
        let oldLit = litCount(value, count: count)
        let newLit = litCount(next, count: count)
        let duration = Double(abs(newLit - oldLit)) * ctx["step"] + 0.15
        previousLit = oldLit
        withAnimation(.linear(duration: duration)) { value = next }
