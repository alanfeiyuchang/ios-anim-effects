...
extension Effect {
    static let inputsVelocitySlider = Effect(
        ...
        prompt: L(
            "A slim 8 pt track with a gradient fill and a white 28 pt thumb. While dragging, the thumb grows to 115% and a rounded value bubble with a small pointer rises 10 pt above it, showing the percentage in monospaced digits. The bubble hangs from its pointer like a pendulum: horizontal drag velocity tilts it against the motion (up to ±30°) and stretches it slightly wider and shorter, chased by a loose spring (response 0.3 s, damping 0.5). When the finger stops or lifts, the bubble swings back upright with a visible wobble (damping 0.45) and settles; a selection haptic ticks every 10%. Playful and physical, yet the number stays perfectly legible.",
            "8pt 细轨道配渐变填充，白色 28pt 圆形滑块。拖动时滑块放大到 115%，上方升起一个带小尖角的圆角数值气泡（上移 10pt），用等宽数字显示百分比。气泡像挂在尖角上的钟摆：水平拖动速度让它逆着运动方向倾斜（最多 ±30°），并被略微横向拉宽、纵向压扁，由较松的弹簧（响应 0.3 秒、阻尼 0.5）追随。手指停下或抬起时，气泡带着明显的摇摆（阻尼 0.45）回正并静止；每跨过 10% 触发一次选择触觉。俏皮而有物理感，数字始终清晰可读。"
        ),
        implementation: L(
