...
extension Effect {
    static let inputsEmojiFaceRating = Effect(
        ...
        prompt: L(
            "A feedback card with a single 120 pt face above a five-label slider (Awful → Great). Dragging morphs the face continuously: the mouth is one Bézier curve whose control points move from a deep frown to a wide grin, the eyes squash into happy arcs at the top end and droop at the bottom, the brows tilt from worried to relaxed, and the face colour blends coral → amber → mint. The slider thumb follows the finger, and on release it snaps to the nearest of five levels on a bouncy spring (response 0.45 s, damping 0.6), so the mouth overshoots its final shape; each time the level changes the face bobs up 6 pt with a keyframed hop and the label beneath swaps with a push transition. A selection haptic ticks per level. Expressive, empathetic and impossible to misread.",
            "反馈卡片上方是一张 120pt 的脸，下方是带五个标签（很差 → 很棒）的滑块。拖动时脸部连续形变：嘴巴是一条贝塞尔曲线，控制点从深深的皱眉移动到咧嘴大笑；眼睛在高分端被压成开心的弧线、在低分端下垂；眉毛从担忧的倾斜变为放松；脸色由珊瑚红经琥珀过渡到薄荷绿。滑块跟随手指，松手后以弹性弹簧（响应 0.45 秒、阻尼 0.6）吸附到最近的五档之一，嘴形会先过冲再定型；每换一档，脸会以关键帧向上轻跳 6pt，下方标签以推入过渡切换，并触发一次选择触觉。表达丰富、有同理心，绝不会被误读。"
        ),
        implementation: L(
...
private struct FeelingFace: View, Animatable {
    ...
    private var features: some View {
        let tilt: Double = (0.5 - mood) * 40
        let eyeHeight: CGFloat = 16 - CGFloat(max(0, mood - 0.6)) * 22
        let ink = Color.black.opacity(0.72)
        return VStack(spacing: 14) {
            HStack(spacing: 30) {
                eye(tilt: tilt, height: eyeHeight, ink: ink, mirrored: false)
                eye(tilt: -tilt, height: eyeHeight, ink: ink, mirrored: true)
            }
            MoodMouth(mood: mood)
    ...
    private func eye(tilt: Double, height: CGFloat, ink: Color, mirrored: Bool) -> some View {
        VStack(spacing: 6) {
            Capsule()
                .fill(ink)
