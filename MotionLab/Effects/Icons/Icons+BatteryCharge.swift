...
extension Effect {
    static let iconsBatteryCharge = Effect(
        ...
        prompt: L(
            "A large horizontal battery outline (150×70 pt, 3 pt stroke, rounded, with a small terminal cap) sits at 18%, its fill red. Tapping plugs it in: a cable plug slides 60 pt from the right into the cap on a snappy spring (≈0.35 s); 250 ms later a lightning bolt pops into the centre from 20% to 100% scale on a bouncy spring and keeps pulsing; then the fill grows to 100% over 2.4 s with ease-in-out, its colour stepping from red (under 20%) to amber (under 45%) to green, while the percentage counts up inside and a soft highlight sweeps across the fill every 1.3 s. A success haptic lands at full. Tapping again unplugs, the bolt shrinks away and the level eases back to 18%. Clear, reassuring feedback.",
            "一个大号横向电池轮廓（150×70 pt、3 pt 描边、圆角，带一个小正极帽）显示 18% 电量，填充为红色。点击即插上电源：插头以利落的弹簧（约 0.35 秒）从右侧 60 pt 外滑入正极帽；250 毫秒后，中央的闪电以有弹性的弹簧从 20% 放大到 100% 并持续脉动；随后电量在 2.4 秒内以缓入缓出涨到 100%，颜色从红色（低于 20%）变为琥珀色（低于 45%）再变为绿色，内部的百分比同步计数，一道柔和的高光每 1.3 秒扫过填充。充满时伴随成功触感。再次点击则拔下插头，闪电缩小消失，电量缓缓回到 18%。清晰、令人安心的反馈。"
        ),
        implementation: L(
