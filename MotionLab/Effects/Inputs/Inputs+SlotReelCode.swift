...
extension Effect {
    static let inputsSlotReelCode = Effect(
        ...
        prompt: L(
            ...
            "六个 42 × 54pt 的验证码格子，下方是 iOS 风格的自动填充建议胶囊（“来自信息 · 482 913”）。点击胶囊后，胶囊收缩为一个小小的“重置”按钮，每个格子像老虎机转轮一样旋转：一列数字向上滚过两个完整循环，再以弹簧（响应 0.6 秒、阻尼 0.78）停在目标数字上，略微过冲后回稳。转轮从左到右以 110 毫秒错峰依次停下，每停一格触发一次选择触觉，格子从 97% 弹回原大，边框加粗变为靛蓝。最后一个转轮停稳后，边框依次由灰变绿扫过，并以一次成功触觉确认。每个格子的上下边缘会淡化经过的数字，让转轮看起来像弯曲的滚筒。令人愉悦，也让“无形”的自动填充显得来之不易。"
        ),
        implementation: L(
