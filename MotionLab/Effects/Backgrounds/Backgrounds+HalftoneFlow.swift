...
extension Effect {
    static let backgroundsHalftoneFlow = Effect(
        ...
        prompt: L(
            "A dark graphite canvas covered edge to edge by a regular halftone dot matrix. Each dot's radius is driven by a smooth interference field — the sum of two diagonal traveling sine waves, one horizontal-drifting wave and one ripple radiating from the center, each moving at a different speed (4–10 s periods) — so bands of large dots glide, cross and dissolve like light under water, while quiet areas shrink to pinpoints. The whole matrix is tinted by a single diagonal gradient (indigo → violet → pink → amber) so color stays still while form moves. It replaces film grain with something crisp and intentional: technical, calm and quietly premium, in the spirit of Stripe and Vercel hero art.",
