...
extension Effect {
    static let buttonsLikeBurst = Effect(
        ...
        prompt: L(
            "An outlined heart inside a soft circular chip, with a like counter below. On tap the outline swaps to a filled pink-to-coral heart with a symbol replace transition while the heart squashes to 60% in 100 ms, overshoots to 125% and settles at 100% on a bouncy spring. Simultaneously a pink shock ring expands from 40 to 150 pt while its stroke thins and fades (~450 ms), and 10 multicolored dots shoot radially outward about 70 pt on an ease-out curve, shrinking and fading after 300 ms. The counter rolls up one digit and a success haptic fires. Un-liking simply swaps back without the burst. It feels celebratory, joyful and rewarding.",
            "柔和圆形底座中放置一个空心爱心，下方显示点赞数。点击后，空心图标以符号替换过渡变为粉到珊瑚色的实心爱心，同时爱心在 100 毫秒内压缩到 60%，再以弹性弹簧过冲到 125% 后回落至 100%。与此同时，一圈粉色冲击环从 40pt 扩散到 150pt，描边逐渐变细并淡出（约 450 毫秒）；10 颗彩色小圆点以缓出曲线沿径向飞出约 70pt，300 毫秒后开始缩小并消失。计数向上滚动一位，并触发成功触觉。取消点赞仅切回空心、不播放爆发。整体欢快、有庆祝感，令人愉悦。"
        ),
        implementation: L(
