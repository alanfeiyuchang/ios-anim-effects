...
extension Effect {
    static let chartsLineDraw = Effect(
        ...
        prompt: L(
            ...
            "一张 290 × 170pt 的营收折线图，带四条标注 $8k～$26k 的淡色水平参考线，下方隔月标注月份（1 月～9 月）：经 Catmull-Rom 平滑的曲线（3pt，圆角端点与连接，靛蓝 → 紫 → 粉渐变），下方面积填充从曲线处 35% 靛蓝渐隐到基线透明。出现时与点击时，图表由一条从左向右扫过的裁切边揭示，历时 1.6 秒，曲线为 ease-in-out（0.65, 0, 0.35, 1）；曲线与面积共用同一裁切边，严丝合缝。一个 10pt 白边圆点带着柔和彩色光晕，精确贴合裁切边处的曲线位置移动，并携带一个数值胶囊实时滚动。重播时先在 250ms 内倒带收回，换入新数据后再次绘制。优雅而富有叙事感。"
        ),
        implementation: L(
