...
extension Effect {
    static let backgroundsBokeh = Effect(
        ...
        prompt: L(
            ...
            "昏暗的梅子色到酒红色渐变背景上，漂浮着柔和发光的失焦光斑，如大光圈镜头下的城市夜灯。光斑分布在三个景深层：远景小、暗且更模糊，近景大、边缘更亮且略清晰。光斑中心淡、边缘最亮，模拟真实散景。所有光斑缓慢上升（依景深不同，穿越画面需 12–30 秒），伴随轻微左右摇摆与 4–7 秒的明暗呼吸，重叠处叠加增亮。左右拖动可移焦：向左让远景变清晰，向右让近景清晰；每层的模糊程度取决于它与焦平面的距离，约 0.3 秒平滑过渡，松手后焦点缓缓回到近景。上方的磨砂玻璃卡片展示使用语境。温暖而亲密。"
        ),
        implementation: L(
