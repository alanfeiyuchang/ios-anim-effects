...
private struct TextSpringChainDemo: View {
    ...
    /// True while a real finger drags the word.
    @State private var held = false
    /// The scripted pull, cancelled on the first real touch.
    @State private var script: Task<Void, Never>?
    /// Resets on system cancellation too, so a stolen touch never leaves the word stretched.
    ...
    var body: some View {
        VStack(spacing: 30) {
            // Only a padded band around the word claims touches, so swipes elsewhere still scroll the page.
            word
                .frame(width: 320, height: 140)
                .contentShape(Rectangle())
                .gesture(dragGesture)
            DemoHint(text: L("Drag the word", "拖动单词"), ctx: ctx)
        }
    ...
    /// Release or system cancellation: the letters spring home one after another.
