...
extension Effect {
    static let iconsSearchClose = Effect(
        ...
        prompt: L(
            "A round search button holds a stroked magnifier (lens plus handle, 3 pt round caps). Tapping it expands the button into a 270 pt search pill while the glyph morphs into a close icon in one continuous stroke: the lens ring unwinds from the handle's side until it disappears, the handle slides and lengthens into the ‘\\’ diagonal of an X, and — from 40% progress — the opposite ‘/’ diagonal grows outward from the centre. It runs on a spring (response 0.5 s, damping 0.78) together with the pill's width, while a placeholder and a blinking caret fade in. Tapping the X plays the morph backwards and collapses the field. A light haptic accompanies both. Economical and clever: the icon itself explains the state change.",
            "一个圆形搜索按钮里是描边放大镜（镜圈加手柄，3 pt 圆头）。点击后按钮展开成 270 pt 宽的搜索胶囊，同时图标用一笔连续的变形变成关闭图标：镜圈从手柄一侧回卷直到消失，手柄滑动并拉长成 X 的「\\」对角线；进度到 40% 时，另一条「/」对角线从中心向两端长出。整个过程与胶囊宽度一起使用弹簧（响应 0.5 秒、阻尼 0.78），占位文字和闪烁光标随之淡入。点击 X 时变形倒放、输入框收起。两次操作都伴随轻触感。简洁巧妙：图标自己讲清了状态的变化。"
        ),
        implementation: L(
