...
extension Effect {
    static let scrollCollapsingHeader = Effect(
        ...
        prompt: L(
            "A screen opens with a 150 pt expanded header: a 64 pt gradient avatar, a 30 pt bold large title and a secondary subtitle. As the content scrolls up, the header is scrubbed continuously over the first 90 pt of scroll (the collapse distance): its height shrinks to 60 pt, the avatar shrinks to 30 pt, the title scales down to ~57% while sliding up and right to sit beside the avatar, and the subtitle fades out twice as fast as the scroll. In parallel a frosted material background and a hairline divider fade in underneath so the rows passing beneath stay legible. Scrolling back down reverses everything exactly, and if the finger lifts mid-collapse the list eases (0.35 s) to the nearer fully expanded or collapsed state. The header itself is pure geometry — no timers — so it feels glued to the finger, like a native iOS navigation bar with more character.",
            "页面顶部是一个 150 pt 的展开式标题区：64 pt 渐变头像、30 pt 粗体大标题与次级副标题。内容上滑时，标题区在前 90 pt 的滚动（折叠距离）内被连续驱动：高度收缩到 60 pt，头像缩小到 30 pt，标题缩小到约 57% 并向右上滑动到头像旁边，副标题以两倍于滚动的速度淡出。与此同时，磨砂材质背景与细分隔线在下方淡入，让从下面经过的列表行依旧清晰可读。向下滚动时所有变化精确反向；若在折叠中途松手，列表会用 0.35 秒平滑停靠到更近的完全展开或完全折叠状态。标题区本身不依赖任何计时器，纯几何驱动，因此如同黏在指尖，像原生 iOS 导航栏，却更有个性。"
        ),
        implementation: L(
        ...
        params: [
            ...
            .slider("range", L("Collapse distance", "折叠距离"), 50...160, default: 90, step: 5, decimals: 0, unit: "pt"),
        ]
    ) { ctx in
