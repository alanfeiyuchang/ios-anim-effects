...
extension Effect {
    static let loadingTraceButton = Effect(
        ...
        prompt: L(
            "A 230 × 56 pt outlined capsule labeled 'Deploy' with a hairline border. On tap the label changes to 'Deploying…' and pulses between 55% and 100% opacity, while a comet — a 28%-long segment of the outline with a transparent tail and a bright sky → violet head, 3 pt wide and softly blurred beneath — laps the border once every 1.2 s. When the job finishes, the comet's tail stops and its head keeps drawing until the whole outline is traced in green over 0.5 s, the label rolls to 'Live' with a check, a faint green fill fades in, and a success haptic fires. Technical, focused, premium.",
            "一枚 230 × 56 pt 的描边胶囊按钮，细线边框，写着“部署”。点击后文字变为“正在部署…”并在 55% 与 100% 透明度间脉动；同时一道彗星光——占轮廓 28% 长度、尾部透明、头部为明亮的天蓝 → 紫罗兰、线宽 3 pt、下方带柔和模糊——每 1.2 秒沿边框绕行一圈。任务完成时彗星尾巴停住，头部继续前进，在 0.5 秒内把整圈轮廓描成绿色；文字滚动为带对勾的“已上线”，淡淡的绿色底色浮现，并伴随成功触感。技术感、专注、高级。"
        ),
        implementation: L(
