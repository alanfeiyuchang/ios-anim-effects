...
extension Effect {
    static let chartsDonutToBars = Effect(
        ...
        prompt: L(
            ...
            "一张“各平台会话”卡片，五个分段（靛蓝、粉、琥珀、薄荷、天蓝）可在原位切换图表类型。环形图状态下每段是一段圆弧（段间约 6° 间隙，内半径为外半径的 60%），中心显示本周总数“11.1k 次会话”；柱状图状态下每段是一根顶部 6pt 圆角、高度等于数值的柱子。点击后每段沿自身轮廓形变：外弧拉直成圆角柱顶，内弧展开成柱底，视线可一路追踪每个数值。各段间隔 60ms 启动，乘着弹簧（响应约 0.7 秒、阻尼约 0.78）并带轻微过冲；总数淡出，各柱数值（4.2k、2.7k……）与基线随之淡入；反向操作再把柱子卷回圆环，巧妙而连贯。"
        ),
        implementation: L(
