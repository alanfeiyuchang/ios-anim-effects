...
extension Effect {
    static let gesturesPhotoViewer = Effect(
        ...
        prompt: L(
            "A 300 × 340 pt landscape photo sits in a rounded viewport. Pinching zooms around the point between the fingers, so that spot stays under them; beyond 1×–4× the scale is rubber-banded and springs back on release. Once zoomed, a pan moves the photo 1:1 and, on release, keeps gliding in the flick's direction with exponential deceleration (≈ 0.3 s time constant); edges resist with rubber-band tension and settle back inside on a spring (response 0.45 s, damping 0.85). Double-tapping zooms to 2.5× centered on the tap, or back to fit, on a spring (response 0.4 s, damping 0.82) with a light haptic, while a small “2.5×” chip counts the level. The photo is re-rendered at every scale, so tiny details — a signpost, cabin windows, stars — stay razor sharp. Familiar, precise and physical.",
            "一张 300 × 340pt 的风景照片置于圆角视窗中。双指捏合时以两指之间的点为中心缩放，该点始终停在指下；超出 1×～4× 的范围时带橡皮筋阻尼，松手后弹回。放大后单指平移 1:1 跟手，松手时沿甩动方向继续滑行并指数减速（时间常数约 0.3 秒）；拖到边缘会受到橡皮筋阻力，并以弹簧（响应 0.45 秒、阻尼 0.85）回到画面之内。双击以点击处为中心放大到 2.5×，再次双击则恢复适配，弹簧参数为响应 0.4 秒、阻尼 0.82，并伴随轻触感；角落的小胶囊实时显示“2.5×”等倍率。照片在每个倍率下都重新绘制，路牌、小屋窗户与星星等细节始终锐利。熟悉、精准、富有物理感。"
        ),
        implementation: L(
