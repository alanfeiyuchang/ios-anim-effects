...
extension Effect {
    static let showcaseFlightPath = Effect(
        ...
        prompt: L(
            "On a dark night-landscape card under a bold “Your Next Adventure Starts Here” headline, a dotted white route (1.5 pt, 2/6 dash) arcs across the card as an S-shaped cubic Bézier. On tap, an airplane glyph flies the curve in ~3.2 s with smoothstep ease-in-out. It stays rotated to the curve’s tangent and swells to 125% mid-flight as if climbing, while a distance counter ticks up. Behind it, a 24-segment orange trail covers the last ~22% of the path and tapers in width (1→3.5 pt) and opacity toward the tail. On arrival the plane fades out over 400 ms. A map pin falls 70 pt with quadratic gravity in 280 ms, bounces back with an exponentially damped rebound (~18 pt, decay 5/s) and sends out one expanding ripple ring. The feel is cinematic, aspirational and precise.",
            "暗色夜景卡片上压着一行粗体标题「你的下一场冒险，从这里出发」，一条白色点状航线（1.5pt，虚线 2/6）以 S 形三次贝塞尔曲线横跨卡片。点击后，飞机图标沿曲线飞行约 3.2 秒，采用 smoothstep 缓入缓出。机身始终对准曲线切线方向，飞到中段放大到 125%，像在爬升，同时里程数字不断累加。机尾拖着一条由 24 段组成的橙色尾迹，覆盖航线最后约 22%，越往后越细（3.5→1pt）、越透明。抵达后飞机在 400 毫秒内淡出。图钉从 70pt 高处按二次曲线下落（280 毫秒），再以指数衰减弹跳（约 18pt，衰减 5/秒），同时扩散出一圈涟漪。整体有电影感，让人向往出发，而且每一步都精确可控。"
        ),
        implementation: L(
            "A TimelineView drives elapsed time; position and heading come from the parametric cubic Bézier and its derivative (atan2), the route and tapered trail are sampled polylines in a Canvas, and the pin's fall and bounce are closed-form functions of time.",
            "TimelineView 提供经过时间；飞机位置与朝向由参数化三次贝塞尔及其导数（atan2）计算，航线与渐细尾迹在 Canvas 中以采样折线绘制，图钉下落与弹跳是关于时间的解析函数。"
        ),
