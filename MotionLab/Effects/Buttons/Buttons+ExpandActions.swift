...
extension Effect {
    static let buttonsExpandActions = Effect(
        ...
        prompt: L(
            ...
            "一张笔记页面卡片，右下角固定一个 60pt 的靛紫渐变悬浮按钮。点击后加号旋转 135° 变成关闭符号，按钮下沉到 92% 再回弹；三个 48pt 的圆形操作（扫描、照片、笔记，各有色调）从按钮中心迸出，沿半径 100pt 的四分之一弧线向左上方扇形展开。每个操作从 30% 缩放到 100% 并淡入，以弹簧（响应 0.4 秒、阻尼 0.7）移动，彼此错开 40 毫秒；收起时顺序反转。与此同时列表内容在 350 毫秒内缩到 95%、模糊 6pt、透明度降到 50%，点击列表即可收起。选中某个操作后扇面收回，一条新笔记带着短暂高亮从列表顶部滑入，并伴随成功触觉。俏皮又有秩序。"
        ),
        implementation: L(
