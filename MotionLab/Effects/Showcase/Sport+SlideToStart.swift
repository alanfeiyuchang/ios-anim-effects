...
extension Effect {
    static let showcaseSlideToStart = Effect(
        ...
        prompt: L(
            "A dark 64 pt capsule track reads \"Start Run\" with a slow shimmer sweeping across it, and a white 52 pt knob carries an arrow. As the knob is dragged, an orange gradient fill stretches behind it, the label fades out at 1.6× the drag progress, and a selection haptic ticks at evenly spaced detents. Past the commit threshold the knob locks to the end (spring 0.35 s, damping 0.8) and its arrow symbol-replaces into a checkmark with a success haptic; 0.7 s later the whole pill floods orange, grows to 103% and reveals a pulsing live dot, \"Run started\" and a ticking timer, while the knob turns into a stop button. Let go early and the knob rubber-bands past either end, then springs home with a small bounce. Physical, decisive and rewarding.",
            "深色 64pt 胶囊轨道上写着“开始滑行”，文字上有一道缓慢扫过的高光；左侧是 52pt 白色圆钮，内含箭头。拖动圆钮时，橙色渐变填充紧随其后拉伸，文字按 1.6 倍拖动进度淡出，每经过一个等距刻度触发一次选择触感。越过触发阈值后，圆钮吸附到末端（弹簧 0.35 秒、阻尼 0.8），箭头以符号替换动画变为对勾并伴随成功触感；0.7 秒后整条胶囊被橙色填满并放大到 103%，展开为脉冲直播点、“已开始滑行”与跳动的计时器，圆钮同时变为停止按钮。提前松手时两端带橡皮筋阻尼，圆钮带轻微回弹地弹回原位。手感扎实、果断，又有完成的满足感。"
        ),
        implementation: L(
