...
extension Effect {
    static let chartsBulletKPI = Effect(
        ...
        prompt: L(
            ...
            "一张目标卡片，包含三条子弹图（营收、NPS、可用率）：每行有三段灰色定性区间（60 / 80 / 100%）、一条 10pt 高的实绩胶囊和一根 3 × 26pt 的目标标记。刷新时，实绩条以减速曲线（0.2, 0.8, 0.2, 1，1.2 秒）从零开始生长，行间错开 150ms。目标标记依据的是动画中实时插值的进度条，而不是最终数值：进度条越过目标的那一帧，标记立即变绿，并以弹性弹簧（响应 0.3 秒、阻尼 0.45）弹跳着放大到 1.5 倍，同时触发一次轻触感；未达标的行保持琥珀色。百分比随进度条同步计数，汇总胶囊滚动为“3 项中 2 项达标”。激励人心、诚实、易于扫读。"
        ),
        implementation: L(
