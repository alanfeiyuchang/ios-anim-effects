...
extension Effect {
    static let chartsSparklineStream = Effect(
        ...
        prompt: L(
            ...
            "三张纵向排列的监控卡片（CPU、内存、网络），每张含说明文字、圆体等宽的实时百分比和一条高 54pt 的迷你折线，分别为靛蓝、薄荷绿、珊瑚色，下方配柔和的竖向渐变填充。新样本每 0.4 秒由均值回归的随机游走生成，但图表从不跳动：整条序列以每个间隔一个样本宽度的速度持续左移，最新点从右边界外滑入；领头圆点（7pt 实心 + 14pt 半透明光晕）固定在右缘，高度在最后两个样本之间插值。百分比读取同一插值，平滑而不跳变。点击卡片会注入一个峰值，随下一个样本进入，再随游走慢慢回落。沉静而始终在线。"
        ),
        implementation: L(
