...
Motionary 的 90 秒宣传片（`MotionLab/Trailer/`）是 App 里实时渲染的「一镜到底」动画：启动参数 `-ML_trailer YES` 让 App 播放它，
...
| 时间 | 场景 | 画面 | 旁白（默认） |
| --- | --- | --- | --- |
| 0–6 s | 开场 | 动效总数「461」用 App 里「翻滚棱柱数字」效果的真实 3D 棱柱一面面翻滚，从左到右带回弹落定；下方「个 iOS 高级动效」和两个统计胶囊 | 做 App 的时候，你一定遇到过这种情况—— |
| 6–16 s | 痛点 1 | 「想要的动效，说不出来」，周围漂浮「弹一下？」「顺滑一点？」「像果冻？」…… | 脑子里有感觉，却说不清…… |
| 16–24 s | 痛点 2 | 一格格模糊的未知动效，每格一个问号：「iOS 能做到什么？不知道」 | 更难的是，你根本不知道 iOS 原生能做出哪些效果。 |
| 24–30 s | 登场 | App 图标 + Motionary + 动效词典，四个卖点：找灵感 · 上手感受 · 实时可调 · 一键复制提示词 | 所以我做了 Motionary…… |
| 30–44 s | 找灵感 | 手机里是 **App 真实的首页和搜索页**（像录屏）：首页滑过精选、系列、分类；点搜索，输入「卡片」，点「质感交互精选」分类，结果里都是实时播放的质感交互动效；再点开一个 | 461 个动效，按 15 个分类、85 个家族整理…… |
| 44–60 s | 上手感受 | 动效详情页依次演示 4 个质感交互动效（照片卡片播放、登顶徽章、收藏迸发、雪板翻转），手指点击带涟漪和「触感」标记 | 每个动效都是真实运行的 SwiftUI，不是视频…… |
| 60–70 s | 实时可调 | 舞台换成弹簧实验，手指拖动阻尼、响应滑块，曲线实时变化，最后停在系统默认弹簧（响应 0.55、阻尼 0.825） | 弹簧的响应时间、阻尼比，拖一下马上看到变化…… |
| 70–80 s | 一键复制提示词 | 提示词卡片、中/EN 切换、一键复制，飞进 AI 对话 | 每个动效都配有中英双语的专业提示词…… |
| 80–87 s | 网页 + App | 左边浏览器（地址栏打出 `alanfeiyuchang.github.io/ios-anim-effects`），右边手机被手指点着玩：「网页随时看 · App 感受手感」 | 想快速浏览，打开网页就能看全部录像…… |
| 87–90 s | 片尾 | 两者收拢成 App 图标：Motionary · 动效词典 · iOS Motion Dictionary + 标语 | ……Motionary，让动效说得清、看得见、摸得着。 |
...
- 页面按场景分组，每组标着它在片中的时间段（例如「找灵感 · 一搜即达 30–44 s」）。
...
把 `trailer/copy.json` 复制进 App 的 `Documents/trailer-copy.json` → 启动并录屏 → 找到白色开场板结束的位置，精确裁出 90 秒 →
...
```
out/9x16/trailer.mp4             成片：1080×1920，60 fps，约 90 秒（直接上传）
...
- 格式就是 `trailer/copy.json` 的样子：按场景分组（`hook`、`pain`、`unknown`、`intro`、`search`、`phone`、`tune`、`prompt`、`chat`、`web`、`end`、`touch`），
  外加旁白数组 `voiceover`。
