...
extension Effect {
    static let morphShapeMorph = Effect(
        ...
        prompt: L(
            "A 200 pt gradient shape floats over a blurred glow of its own silhouette that breathes on a slow loop (opacity 40% ↔ 70%, scale 92% ↔ 104%, 1.6 s ease-in-out each way). Each tap morphs the outline to the next form (circle, squircle, organic blob, six-petal flower) by interpolating 180 radial sample points, so edges flow instead of cross-fading. The morph rides a spring (0.8 s, bounce 0.25) that slightly overshoots into the next form; meanwhile the shape turns 45°, its hue drifts 40° and the glow follows. All forms share one mean radius, so nothing pops in size, and a caption naming the shape blur-replaces. Liquid, sculptural, alive.",
            "约 200 pt 的渐变图形浮在自己的模糊辉光之上，辉光按慢节奏呼吸（透明度 40% ↔ 70%，缩放 92% ↔ 104%，单程 1.6 秒缓入缓出）。每点一下，轮廓就流向下一个形态：圆、超椭圆、有机液滴、六瓣花，靠 180 个径向采样点逐点插值，边缘连续流淌而非交叉淡变。形变乘着弹簧（约 0.8 秒、回弹 0.25），略微冲过目标再回落；同时图形转过 45°，色相偏移 40°，辉光紧随轮廓。四种形态平均半径相同，切换时不会忽大忽小，下方形状名以模糊替换更新。流动、雕塑感、生机勃勃。"
        ),
        implementation: L(
