...
extension Effect {
    static let gesturesVerletRope = Effect(
        ...
        prompt: L(
            "A 52 pt charm (sunset gradient, white star, soft colored shadow) hangs from a small pin by a ~190 pt cord. The cord is 14 Verlet links under ≈ 1500 pt/s² gravity with light air drag, solved 10 times per 1/120 s step, and the charm is about three times heavier than a link, so the cord stays taut under its weight. Grabbing the charm pins it to the finger (never beyond the cord's reach); the cord drapes and curls naturally behind it, and letting go hands over the finger's release velocity, so a flick sends the charm swinging in wide arcs while a wave travels down the cord and it whips around the pin. The charm always rotates to follow the last link. Lower stiffness turns the cord into an elastic bungee. Tactile, playful, physically honest.",
            "一枚 52pt 的吊坠（日落渐变、白色星形、柔和同色投影）通过约 190pt 的绳子挂在一枚小钉上。绳子由 14 节 Verlet 链段组成，受约 1500pt/s² 的重力与轻微空气阻力，每个 1/120 秒步长求解 10 次约束；吊坠质量约为单节的三倍，因此绳子在其重量下保持绷直。抓住吊坠即将其固定在指尖（不会超出绳长），绳子在身后自然垂坠、卷曲；松手时继承手指的离手速度，轻轻一甩，吊坠便大幅摆荡，一道波沿绳传递，绳子绕着钉子鞭梢般甩动。吊坠始终沿最后一节绳子旋转。降低刚度，绳子会变成富有弹性的蹦极绳。可触、俏皮、物理真实。"
        ),
        implementation: L(
